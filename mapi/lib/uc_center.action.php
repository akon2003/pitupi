...
class uc_center
{
	public function index(){
		...
		if ($user_id >0){
			...
			$self_money = $GLOBALS['db']->getOne("select sum(d.self_money) from ".DB_PREFIX."deal_load_repay d left join ".DB_PREFIX."user u on u.id=d.user_id where u.id=".$user_id." and d.has_repay='0'");//20160802 xiejun 待返本金
			// file_put_contents(dirname(__FILE__).'/uc_center.txt', serialize($self_money));
			if($t_sign_data)
			{			
...
//			$is_peizi =  $GLOBALS['db']->getOne("select val from ".DB_PREFIX."m_config where code = 'is_peizi' ");
//			$root['is_peizi'] = $is_peizi;
			...
			$user['self_money'] = format_price($self_money);//20160802 xiejun 待返本金		
			$user['lock_money_format'] = format_price($user['lock_money']);//冻结资金			
			$user['total_money'] = $user['money'] + $user['lock_money'] + $self_money;//资金余额
			...
			$root['self_money'] = $user['self_money'];
			//Admin 2016/8/22 加入VIP等级
			$root['vip_id'] = $user['vip_id'];
			$root['vip_name'] = get_vip_name($user['vip_id']);
			//我的红包
