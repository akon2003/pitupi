...
class uc_trader_verify
{
	public function index(){
		...
		if ($user_id >0){
			...
			$trader_list = $GLOBALS['db']->getAll("select po.*,pc.name as conf_type_name from ".DB_PREFIX."peizi_order po left join ".DB_PREFIX."peizi_conf pc on po.peizi_conf_id = pc.id where $l_condition and  po.user_id = ".intval($GLOBALS['user_info']['id'])." order by order_sn desc limit ".$limit);		
			
			$trader_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."peizi_order where $c_condition and  user_id = ".intval($GLOBALS['user_info']['id']));		
