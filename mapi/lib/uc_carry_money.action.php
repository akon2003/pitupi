...
class uc_carry_money
{
	public function index(){
		...
		if ($user_id >0){
			...
			$bank_list = $GLOBALS['db']->getAll("SELECT u.id,u.bank_id,u.bankcard,u.real_name, b.name as bank_name FROM ".DB_PREFIX."user_bank u left join ".DB_PREFIX."bank b on b.id = u.bank_id where u.user_id=".$user_id."  AND u.id=".$bid."  ORDER BY u.id ASC");
			foreach($bank_list as $k=>$v){
				...
				$bank_list[$k]['uimg'] = str_replace("/mapi","",SITE_DOMAIN.APP_ROOT.'/public/bank/'.$v['bank_id'].'.jpg');
			}
			
			$root['bank_carry'] = $bank_list;

			$root['money'] = number_format($user['money'],2);
