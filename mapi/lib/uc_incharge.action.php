	...
	public function index(){
		...
		if ($user_id >0){
			...
			if($GLOBALS['request']['from']=="wap"){
				$payment_list = $GLOBALS['db']->getAll("select id, name as class_name,class_name  as  iclass_name,name,fee_amount,description,logo,fee_type,config from ".DB_PREFIX."payment where is_effect = 1 and online_pay = 2 order by sort desc");
			}else{
			    $not_in = "'Bfrzwap','Wwxjspay'";
				$payment_list = $GLOBALS['db']->getAll("select id, name as class_name,class_name  as  iclass_name,name,fee_amount,description,logo,fee_type,config from ".DB_PREFIX."payment where is_effect = 1 and (online_pay = 2 or online_pay = 3) AND class_name not in(".$not_in.") order by sort desc");
			}
