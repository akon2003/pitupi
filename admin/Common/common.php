...
// 获取用户名
function get_user_name_only($user_id)
{
	$user_info =  M("User")->where("id=".$user_id." and is_delete = 0")->Field("user_name,user_type")->find();
...
// 获取用户名
function get_user_name_iframe($user_id)
{
	$user_info =  M("User")->where("id=".$user_id." and is_delete = 0")->Field("id,user_name,user_type,real_name")->find();
	
	if(!$user_info) {
		return l("NO_USER");
	} else {
		$url = u("User/view",array("user_id"=>$user_info['id']));
		$title = $user_info['user_name'].'['.$user_info['real_name'].']详细信息';
		$func = 'openWeeboxFrame("'.$url.'","'.$title.'",1000,600);return false;';
		return "<a class='indes' href='javascript:void(0);' onclick='".$func."' target='_parent'>".$user_info['user_name']."</a>";
	}
}
// 获取用户名
function get_user_name_dialog($user_id)
{
	$user_info =  M("User")->where("id=".$user_id." and is_delete = 0")->Field("user_name,user_type,real_name")->find();
	
	if(!$user_info) {
		return l("NO_USER");
	} else {
		$url = u("User/".($user_info['user_type']==0? "index" : "company_index"),array("user_name"=>$user_info['user_name']));
		$title = $user_info['user_name'].'['.$user_info['real_name'].'] 详细信息';
		$param = '{contentType:"text",showButton:false,title:"'.$title.'",width:600,height:400}';
		$func = '$.weeboxs.open("'.$url.'",'.$param.');syn_data("'.$url.'");return false;';
		return "<a class='indes' href='javascript:void(0);' onclick='".$func."' target='_blank'>".$user_info['user_name']."</a>";
	}
}
// 获取用户真实姓名
function get_user_real_name($user_id)
{
	if ($user_id == 0) 
		return "";
	$user_info =  M("User")->where("id=".$user_id." and is_delete = 0")->Field("AES_DECRYPT(real_name_encrypt,'".AES_DECRYPT_KEY."') as real_name,user_type")->find();
	if(!$user_info)
		"";
	else
		return $user_info['real_name']!="" ? $user_info['real_name']:"";
}
// 获取最后还款日期
function get_deal_end_repay_time($deal_id)
{
	if ($deal_id == 0) 
		return "";
	$end_time = $GLOBALS["db"]->getOne("select max(repay_time) from ".DB_PREFIX."deal_load_repay where deal_id=".$deal_id);
	if(!$end_time)
		return "";
	else
...
function get_user_mobile($user_id)
{
	if ($user_id == 0) 
		return "";
	$user_info =  M("User")->where("id=".$user_id." and is_delete = 0")->Field("AES_DECRYPT(mobile_encrypt,'".AES_DECRYPT_KEY."') as mobile,user_type")->find();
	if(!$user_info)
		return "";
	else
...
// 
function get_user_idno($user_id)
{
	if ($user_id == 0) 
		return "";
	$user_info =  M("User")->where("id=".$user_id." and is_delete = 0")->Field("AES_DECRYPT(idno_encrypt,'".AES_DECRYPT_KEY."') as idno,user_type")->find();
	if(!$user_info)
		return "";
	else
		return $user_info['idno']!="" ? $user_info['idno']:"";
}
// 
function get_refer_user_name($user_id)
{
	if ($user_id == 0) 
		return "";
	$refer_user_id =  M("User")->where("id=".$user_id." and is_delete = 0")->Field("pid")->find();
	if(!$refer_user_id)
		return "";
	else
		return get_user_real_name($refer_user_id['pid']);
}
// 
function get_service_user_name($user_id)
{
	if ($user_id == 0) 
		return "";
	$service_user_id =  M("User")->where("id=".$user_id." and is_delete = 0")->Field("admin_id")->find();
	if(!$service_user_id)
		return "";
	else
		return get_admin_real_name($service_user_id['admin_id']);
}
// 获取用户性别
...
// 获取后台用户真实姓名
function get_admin_real_name($user_id)
{
	if ($user_id == 0)
		return "";
	$user_info =  M("Admin")->where("id=".$user_id)->Field("real_name")->find();
	if(!$user_info)
		"";
	else
		return $user_info['real_name']!="" ? $user_info['real_name']:"";
...
{
	$user_info =  M("User")->where("id=".$user_id." and is_delete = 0")->Field("AES_DECRYPT(money_encrypt,'".AES_DECRYPT_KEY."') as money,user_type")->find();
	if(!$user_info)
		return format_price(0);
	else
		return format_price($user_info['money']!="" ? $user_info['money']:0);
}

// 获取导航类型
function get_nav_type($type_id)
{
	$type_info =  M("Nav")->where("id=".$type_id)->Field("name")->find();
	if(!$type_info)
		return "";
	else
...
// 获取客服人员的投资人数
function get_deal_load_customers($user_id) {
	$sql = "select count(distinct ".DB_PREFIX."deal_load.user_id) as count from ".DB_PREFIX."deal_load, ".DB_PREFIX."user where ".DB_PREFIX."user.id=".DB_PREFIX."deal_load.user_id and ".DB_PREFIX."user.admin_id=".$user_id;
 ...
 * 获取用户所属授权服务中心
 */
function get_authority($authority_id) {
	if ($authority_id == 0) {
		return '';
	}

	$authority_info = $GLOBALS['db']->getOne("select adm_name from ".DB_PREFIX."admin where id=".$authority_id." and is_authority=1");
	if ($authority_info) {
