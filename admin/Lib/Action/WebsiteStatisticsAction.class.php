...
//		var_dump($list);exit;
		...
		if($list)
		{
			...
			if($page == 1)
	    	$content_advance_info = iconv("utf-8","gbk","代还时间,借款名称,第几期,管理员,担保机构,代还本息,代还管理费,代还罚息,代还多少逾期管理费");
	...
	public function website_age_group_total(){
		...
		$sql_str = "select 
		sum(if(($now_year-byear) < 20, 1, 0)) as 20岁以下,
		sum(if(($now_year-byear) >= 20 and ($now_year-byear) < 30, 1, 0)) as 20至30岁,
		sum(if(($now_year-byear) >= 30 and ($now_year-byear) < 40, 1, 0)) as 30至40岁,
		sum(if(($now_year-byear) >= 40 and ($now_year-byear) < 50, 1, 0)) as 40至50岁,
		sum(if(($now_year-byear) >= 50, 1, 0)) as 50岁以上,
		count(*) as totals
		from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 ";

		$model = D();
		
		$voList = $this->_Sql_list($model, $sql_str, "&".$parameter,false);
		$sql_str1 = "select sum(if(($now_year-byear) < 20, 1, 0)) as one from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 ";
		$pieinfo1=$GLOBALS['db']->getOne($sql_str1);
		$sql_str2 = "select sum(if(($now_year-byear) >= 20 and ($now_year-byear) < 30, 1, 0)) as two from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 ";
		$pieinfo2=$GLOBALS['db']->getOne($sql_str2);
		$sql_str3 = "select sum(if(($now_year-byear) >= 30 and ($now_year-byear) < 40, 1, 0)) as three from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 ";
		$pieinfo3=$GLOBALS['db']->getOne($sql_str3);
		$sql_str4 = "select sum(if(($now_year-byear) >= 40 and ($now_year-byear) < 50, 1, 0)) as four from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 ";
		$pieinfo4=$GLOBALS['db']->getOne($sql_str4);
		$sql_str5 = "select sum(if(($now_year-byear) >= 50, 1, 0)) as five from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 ";
		$pieinfo5=$GLOBALS['db']->getOne($sql_str5);
		$sql_strtal = "select count(*) as totals from ".DB_PREFIX."user where is_delete = 0 and user_type in (0,1) and is_effect = 1 ";
		$pieinfotal=$GLOBALS['db']->getOne($sql_strtal);
		...
		$total_array=array(
				array(
					array('20岁以下','时间','20岁以下'),
					array('20至30岁','时间','20至30岁'),
					array('30至40岁','时间','30至40岁'),
					array('40至50岁','时间','40至50岁'),
					array('50岁以上','时间','50岁以上')
				),
		);
		krsort($voList);
		//print_r($pieList);
		$chart_list=$this->get_jx_json_all_pie($pieList,$total_array,$pieinfotal);
		$this->assign("chart_list",$chart_list);
//		dump($chart_list);
//		exit;
		$this->display();
