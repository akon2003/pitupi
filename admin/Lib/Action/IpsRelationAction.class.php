...
class IpsRelationAction extends CommonAction{
	...
	public function repayment_export_csv($page = 1)
	{
		...
		if($list)
		{
	    	...
	    	if($page == 1)
	    	{
		    	$content = iconv("utf-8","gbk","编号,标号,借款名称,还款人,还款本金,罚息/违约金,管理费,逾期管理费,还款总额,还款日期,第三方受理时间");	    		    	
		    	$content = $content . "\n";
	    	}
	...
	public function deal_export_csv($page = 1)
	{
		...
		if($list)
		{
	    	...
	    	if($page == 1)
	    	{
		    	$content = iconv("utf-8","gbk","编号,标号,借款名称,投标人,承接着,还款本金,利息,管理费,罚息/违约金,换困总额,还款状态,还款日,实际还款时间");	    		    	
		    	$content = $content . "\n";
	    	}
			foreach($list as $k=> $v)
			{
	...
	public function back_export_csv($page = 1)
	{
		...
		if($list)
		{
	    	...
	    	if($page == 1)
	    	{
		    	$content = iconv("utf-8","gbk","编号,标号,借款名称,投标人,还款日,实际还款日,已收本息,管理费,利息管理费,罚息/违约金,状态");	    		    	
		    	$content = $content . "\n";
	    	}
			foreach($list as $k=> $v)
			{
