...
class ProjectAction extends CommonAction{
	...
	public function insert() {
		...
		$data['user_id'] = M("User")->where("user_name=".intval($data['user_name']))->getField("id");
	...
	public function del_pay_log()
	{
		...
		if (isset ( $id )) {
				...
				if($info) $info = implode(",",$info);
				$info = "项目ID".$deal_id.":".$info;
				$list = M("ProjectPayLog")->where ( $condition )->delete();				
				if ($list!==false) {					
	...
	public function batch_refund()
	{
		...
		$deal_info = M("Project")->where("id=".$deal_id." and is_delete = 0 and is_effect = 1 and is_success = 0 and end_time <>0 and end_time <".get_gmtime())->find();
		if(!$deal_info)
		{
