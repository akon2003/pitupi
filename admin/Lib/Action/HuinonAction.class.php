...
class HuinonAction extends CommonAction{
	...
	function penalty(){
		...
		$map['id'] = array("exp"," in (SELECT deal_id FROM ".DB_PREFIX."ext_deal_inrepay_repay) ");
	...
	function publish_update(){
		B('FilterString');
		$data = M('ExtDeal')->create ();
		
		$log_info = M('ExtDeal')->where("id=".intval($data['id']))->getField("name");
		//开始验证有效性
		...
		if(D("ExtDeal")->where("deal_sn='".$data['deal_sn']."' and id<>".$data['id'])->count() > 0){
			$this->error("借款编号已存在");
		}
		...
		if($data['deal_status'] == 4){
			if($GLOBALS['db']->getOne("SELECT sum(money) FROM ".DB_PREFIX."ext_deal_load where deal_id=".$data['id']) <floatval($data['borrow_amount'])){
				$this->error("未满标无法设置为还款状态!");
				exit();
			}
		}
		
		if($data['agency_id']!=M("ExtDeal")->where("id=".$data['id'])->getField("agency_id")){
		...
		if (false !== $list) {
			...
			if($data['is_delete']==3){
				...
				if(app_conf("SMS_ON")==1){
					...
					$deal_info = D("ExtDeal")->where("id=".$data['id'])->find();
					...
					$GLOBALS['db']->autoExecute(DB_PREFIX."ext_deal_msg_list",$msg_data); //插入	
				}
			}
			
			//成功提示
			save_log("编号：".$data['id']."，".$log_info."初审更新成功",1);
			$this->assign("jumpUrl",u('Huinon'."/publish"));
			$this->success("初审更新成功");
		} else {
			//错误提示
			$dbErr = M()->getDbError();
	...
	function true_publish_update(){
		...
		if (false !== $list) {
			...
			$this->assign("jumpUrl",u('Huinon'."/true_publish"));
			$this->success("复审更新成功");
		} else {
			//错误提示
			$dbErr = M()->getDbError();
	...
	function third_publish_update(){
		...
		if (false !== $list) {
			...
			$this->assign("jumpUrl",u('Huinon'."/third_publish"));
	...
	function submit_publish_update(){
		...
		if (false !== $list) {
			...
			$this->assign("jumpUrl",u('Huinon'."/submit_publish"));
			$this->success("发布成功");
		} else {
			//错误提示
			$dbErr = M()->getDbError();
	...
	public function three()
	{
		...
		$sql_count = " SELECT count(*) FROM ".DB_PREFIX."ext_deal_repay dl LEFT JOIN ".DB_PREFIX."ext_deal d ON d.id=dl.deal_id WHERE $condition ";
		...
		if($rs_count > 0){
			...
			$sql_list =  " SELECT dl.*,dl.l_key + 1 as l_key_index,d.name,d.cate_id,d.send_three_msg_time,u.user_name,u.mobile FROM ".DB_PREFIX."ext_deal_repay dl LEFT JOIN ".DB_PREFIX."ext_deal d  ON d.id=dl.deal_id left join ".DB_PREFIX."user u on u.id=dl.user_id WHERE $condition ORDER BY $order $sort LIMIT ".$p->firstRow . ',' . $p->listRows;
	...
	public function three_msg(){
		...
		if($rs_count > 0){
			...
			$sql_list =  " SELECT dl.*,dl.l_key + 1 as l_key_index,d.name,d.cate_id,d.send_three_msg_time FROM ".DB_PREFIX."ext_deal_repay dl LEFT JOIN ".DB_PREFIX."ext_deal d  ON d.id=dl.deal_id WHERE $condition ORDER BY dl.repay_time ASC LIMIT ".$p->firstRow . ',' . $p->listRows;
			$list = $GLOBALS['db']->getAll($sql_list);			
		}
		$deal_ids = array();
		//发送信息
		foreach($list as $k=>$v){
			if($v['send_three_msg_time'] != $v['repay_time']){
				...
				$GLOBALS['db']->autoExecute(DB_PREFIX."ext_deal",array("send_three_msg_time"=>$v['repay_time']),"UPDATE","id=".$v['deal_id']); 
			}
		}
	...
	function repay_log(){
		...
		$deal_repay = $GLOBALS['db']->getRow("SELECT dr.*,dr.l_key + 1 as l_key_index,d.name FROM  ".DB_PREFIX."ext_deal_repay dr LEFT JOIN ".DB_PREFIX."ext_deal d ON d.id=dr.deal_id WHERE  dr.id=".$id);
		if(!$deal_repay){
			$this->error("账单不存在");
		}
		
		$sql_count = " SELECT count(*) FROM ".DB_PREFIX."ext_deal_repay_log WHERE repay_id= ".$id;
		...
		if($rs_count > 0){
			...
			$sql_list =  " SELECT * FROM ".DB_PREFIX."ext_deal_repay_log WHERE repay_id= ".$id." ORDER BY id DESC LIMIT ".$p->firstRow . ',' . $p->listRows;
			$list = $GLOBALS['db']->getAll($sql_list);
	...
	public function do_op_status(){
		...
		//VIP 坏账降级 
		$condition['id'] = $id;		
		$voss = M('ExtDeal')->where($condition)->find();
		...
		$GLOBALS['db']->query("UPDATE ".DB_PREFIX."ext_deal_repay SET is_site_bad=".(intval($_REQUEST['is_site_bad']) -1)." WHERE id=".$id);
	...
	function generation_repay(){
		...
		$sql_count = "SELECT count(*) FROM ".DB_PREFIX."generation_repay gr LEFT join ".DB_PREFIX."ext_deal_repay dr ON dr.id=gr.repay_id LEFT JOIN ".DB_PREFIX."ext_deal d ON d.id=gr.deal_id ";
	...
	function op_generation_repay_status(){
		$id = intval($_REQUEST['id']);
		$deal_repay = $GLOBALS['db']->getRow("SELECT gr.*,dr.user_id,dr.l_key + 1 as l_key_index,dr.is_site_bad,d.name FROM ".DB_PREFIX."generation_repay gr LEFT JOIN  ".DB_PREFIX."ext_deal_repay dr ON dr.id=gr.repay_id LEFT JOIN ".DB_PREFIX."ext_deal d ON d.id=dr.deal_id WHERE  gr.id=".$id);
		if(!$deal_repay){
			$this->error("账单不存在");
		}
	...
	public function add()
	{
		$this->assign("new_sort", M("ExtDeal")->where("is_delete=0")->max("sort")+1);
        ...
        $last_deal_sn = $GLOBALS['db']->getOne("select deal_sn from ".DB_PREFIX."ext_deal where ext_name='huinon' order by id desc limit 1");
        if (preg_match("/^HN\d*",$last_deal_sn)) {
            $deal_sn = "HN".str_pad(intval(substr($last_deal_sn,2)) + 1,3,0,STR_PAD_LEFT);
        } else {
            $deal_sn = "HN001";
        }
	...
	public function insert() {
		...
		if(D("ExtDeal")->where("deal_sn='".$data['deal_sn']."'")->count() > 0){
			$this->error("借款编号已存在");
		}
		...
		$data['mortgage_infos'] = $this->mortgage_info();
        $data['mortgage_contract'] = $this->mortgage_info("contract");
        $data['ext_name'] = 'huinon';

		$list=M('ExtDeal')->add($data);
		if (false !== $list) {
			foreach($_REQUEST['city_id'] as $k=>$v){
				if(intval($v) > 0){
			...
			$this->assign("jumpUrl",u('Huinon'."/add"));
	...
	public function edit() {		
		...
		$condition['id'] = $id;		
		$vo = M('ExtDeal')->where($condition)->find();
		...
		if(trim($_REQUEST['type'])=="deal_status"){
			$this->display ("Huinon:deal_status");
			exit();
		}
	...
	public function update() {
		...
		if (false !== $list) {
			...
			$this->assign("jumpUrl",u('Huinon'."/edit",array("id"=>$data['id'])));
			$this->success(L("UPDATE_SUCCESS"));
		} else {
			//错误提示
			$dbErr = M()->getDbError();
	...
	public function delete() {
		...
		if (isset ( $id )) {
				...
				//无法删除的
				$rel_data = M('ExtDeal')->where($condition1)->findAll();				
				...
				//可以删除的
				$rel_data = M('ExtDeal')->where($condition)->findAll();				
				foreach($rel_data as $data)
				{
				...
				$list = M('ExtDeal')->where ( $condition )->setField ( 'is_delete', 1 );
				if ($list!==false) {
	...
	function do_apart(){
		...
		$GLOBALS['db']->autoExecute(DB_PREFIX."ext_deal",$data,"UPDATE","id=".$id);
		...
		if($result['status'] == 1 && (int)$_REQUEST['make_new'] == 1){
			...
			$GLOBALS['db']->autoExecute(DB_PREFIX."ext_deal",$new_data,"INSERT");
			$new_id = $GLOBALS['db']->insert_id();
			if($new_id > 0){
				$deal_sn = "MER".to_date(TIME_UTC,"Y")."".str_pad($new_id,7,0,STR_PAD_LEFT);
				$GLOBALS['db']->query("update ".DB_PREFIX."ext_deal SET deal_sn='".$deal_sn."' WHERE id=".$new_id);
		...
		elseif($result['status'] == 1){
			...
			$GLOBALS['db']->autoExecute(DB_PREFIX."ext_deal",$deal,"UPDATE","id=".$id);
			syn_deal_status($id);
	...
	public function restore() {
		...
		if (isset ( $id )) {
				...
				$list = M('ExtDeal')->where ( $condition )->setField ( 'is_delete', 0 );
				if ($list!==false) {
	...
	public function foreverdelete() {
		...
		if (isset ( $id )) {
				$condition = array ('id' => array ('in', explode ( ',', $id ) ) );
				//删除的验证
				if(M("ExtDealOrder")->where(array ('deal_id' => array ('in', explode ( ',', $id ) ) ))->count()>0)
				{
					$this->error(l("DEAL_ORDER_NOT_EMPTY"),$ajax);
				}
				M("ExtDealPayment")->where(array ('deal_id' => array ('in', explode ( ',', $id ) ) ))->delete();
				M("ExtDealLoad")->where(array ('deal_id' => array ('in', explode ( ',', $id ) ) ))->delete();
				M("ExtDealLoadRepay")->where(array ('deal_id' => array ('in', explode ( ',', $id ) ) ))->delete();
				M("ExtDealRepay")->where(array ('deal_id' => array ('in', explode ( ',', $id ) ) ))->delete();
				M("ExtDealCollect")->where(array ('deal_id' => array ('in', explode ( ',', $id ) ) ))->delete();
				...
				if($info) $info = implode(",",$info);
				$list = M('ExtDeal')->where ( $condition )->delete();	
	...
	public function set_sort()
	{
		...
		$log_info = M('ExtDeal')->where("id=".$id)->getField('name');
		if(!check_sort($sort))
		{
			$this->error(l("SORT_FAILED"),1);
		}
		M('ExtDeal')->where("id=".$id)->setField("sort",$sort);
	...
	public function set_effect()
	{
		$id = intval($_REQUEST['id']);
		$ajax = intval($_REQUEST['ajax']);
		$info = M('ExtDeal')->where("id=".$id)->getField("name");
		$c_is_effect = M('ExtDeal')->where("id=".$id)->getField("is_effect");  //当前状态
		$n_is_effect = $c_is_effect == 0 ? 1 : 0; //需设置的状态
		M('ExtDeal')->where("id=".$id)->setField("is_effect",$n_is_effect);	
		M('ExtDeal')->where("id=".$id)->setField("update_time",TIME_UTC);	
	...
	public function set_new()
	{
		...
		$ajax = intval($_REQUEST['ajax']);
		$info = M('ExtDeal')->where("id=".$id)->getField("name");
		$c_is_effect = M('ExtDeal')->where("id=".$id)->getField("is_new");  //当前状态
		$n_is_effect = $c_is_effect == 0 ? 1 : 0; //需设置的状态
		M('ExtDeal')->where("id=".$id)->setField("is_new",$n_is_effect);	
		M('ExtDeal')->where("id=".$id)->setField("update_time",TIME_UTC);	
		save_log($info.l("SET_EFFECT_".$n_is_effect),1);
	...
	public function set_advance()
	{
		...
		$ajax = intval($_REQUEST['ajax']);
		$info = M('ExtDeal')->where("id=".$id)->getField("name");
		$c_is_advance = M('ExtDeal')->where("id=".$id)->getField("is_advance");  //当前状态
		$n_is_advance = $c_is_advance == 0 ? 1 : 0; //需设置的状态
		M('ExtDeal')->where("id=".$id)->setField("is_advance",$n_is_advance);	
	...
	public function set_hidden()
	{
		...
		$ajax = intval($_REQUEST['ajax']);
		$info = M('ExtDeal')->where("id=".$id)->getField("name");
		$c_is_advance = M('ExtDeal')->where("id=".$id)->getField("is_hidden");  //当前状态
		$n_is_advance = $c_is_advance == 0 ? 1 : 0; //需设置的状态
		M('ExtDeal')->where("id=".$id)->setField("is_hidden",$n_is_advance);	
	...
	public function show_detail()
	{
		...
		$id = intval($_REQUEST['id']);
		syn_deal_status($id);
		$deal_info = M("ExtDeal")->getById($id);
		$this->assign("deal_info",$deal_info);
		
		$true_repay_money  =  M("ExtDealLoadRepay")->where("deal_id=".$id)->sum("repay_money");
		
		$this->assign("true_repay_money",floatval($true_repay_money) + 1);
		
		$count = D("ExtDealLoad")->where('deal_id='.$id)->order("id ASC")->count();
		if (! empty ( $_REQUEST ['listRows'] )) {
			$listRows = $_REQUEST ['listRows'];
		} else {
			$listRows = '';
		...
		if($count>0){
			$loan_list = D("ExtDealLoad")->where('deal_id='.$id)->order("id ASC")->limit($p->firstRow . ',' . $p->listRows)->findall();
	 ...
	 */
	 ...
	 function do_site_repay($page=1){
		...
		$this->assign("jumpUrl",U("Huinon/repay_plan",array("id"=>$id)));
		...
		$get_manage = $GLOBALS['db']->getOne("SELECT get_manage FROM ".DB_PREFIX."ext_deal_repay WHERE deal_id = ".$deal_info['id']." and l_key=".$l_key."  ");
		...
		foreach($user_loan_list['item'] as $kk=>$vv){
			if($vv['has_repay']==0 ){//借款人已还款，但是没打款到借出用户中心
				...
				$GLOBALS['db']->autoExecute(DB_PREFIX."ext_deal_load_repay",$user_load_data,"UPDATE","id=".$vv['id']." AND has_repay = 0 ","SILENT");
				...
				if($GLOBALS['db']->affected_rows() > 0){
					...
					$load_repay_rs = $GLOBALS['db']->getOne("SELECT (sum(true_interest_money) + sum(impose_money)) as shouyi,sum(impose_money) as total_impose_money FROM ".DB_PREFIX."ext_deal_load_repay WHERE deal_id=".$deal_info['id']." AND user_id=".$vv['user_id']);
		...
		if($page >= $page_all){
			...
			if($s_count == 0){
				...
				$GLOBALS['db']->autoExecute(DB_PREFIX."ext_deal_repay",$repay_data,"UPDATE"," deal_id=".$id." AND l_key=".$l_key." and has_repay in (0,2) ");
	 ...
	 function user_loads_repay(){
		...
		$sql_count = "SELECT count(*) FROM ".DB_PREFIX."ext_deal_load_repay dlr LEFT JOIN ".DB_PREFIX."ext_deal d On d.id = dlr.deal_id where $extWhere $condition";
		$rs_count = $GLOBALS['db']->getOne($sql_count);
		...
		if($rs_count > 0){
			...
			$sql_list =  "SELECT dlr.*,dlr.l_key +1 as l_key_index ,dl.money,d.name,d.rate,d.repay_time_type,d.repay_time as all_repay_time FROM ".DB_PREFIX."ext_deal_load_repay dlr LEFT JOIN  ".DB_PREFIX."ext_deal_load dl  ON dl.id = dlr.load_id LEFT JOIN ".DB_PREFIX."ext_deal d On d.id = dlr.deal_id WHERE $condition  ORDER BY $order $sort LIMIT ".$p->firstRow . ',' . $p->listRows;
	...
	function do_loans(){
		...
		$GLOBALS['db']->autoExecute(DB_PREFIX."ext_deal",$loan_data,"UPDATE","id=".$id);
	...
	private function get_manage($id){
		...
		if(intval($_REQUEST['get_manage'])==1){
			require_once(APP_ROOT_PATH."system/libs/user.php");
			require_once(APP_ROOT_PATH."system/common.php");
			$deal_name = $GLOBALS['db']->getOne("select name from ".DB_PREFIX."ext_deal where id='$id' ");
			$deal_repay = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."ext_deal_repay where deal_id='$id' AND has_repay=0 ");
			if($deal_repay){
				foreach($deal_repay as $k=>$v){
					if($v['manage_money']!=0 && $v['get_manage'] == 0){
						$GLOBALS['db']->query("UPDATE ".DB_PREFIX."ext_deal_repay SET true_manage_money = manage_money,get_manage=1 WHERE id=".$v['id']);
						if($GLOBALS['db']->affected_rows() > 0){
							...
							$GLOBALS['db']->query("UPDATE ".DB_PREFIX."ext_deal_load_repay SET true_repay_manage_money = repay_manage_money WHERE repay_id=".$v['id']);
	...
	{	
		...
		$list = M("ExtDealLoad")->limit($limit)->where('deal_id ='.$id)->findAll();
...
public function export_csv_three($page = 1)
	{
		...
		$sql_list =  " SELECT dl.*,dl.l_key + 1 as l_key_index,d.name,d.cate_id,d.send_three_msg_time,u.user_name,u.mobile FROM ".DB_PREFIX."ext_deal_repay dl LEFT JOIN ".DB_PREFIX."ext_deal d  ON d.id=dl.deal_id left join ".DB_PREFIX."user u on u.id=dl.user_id WHERE $condition  ORDER BY $order $sort LIMIT ".$limit;
		$list = $GLOBALS['db']->getAll($sql_list);
	...
	public function export_csv_yuqi($page = 1)
	{
		...
		$list = array();
		$sql_list =  " SELECT dl.*,dl.l_key + 1 as l_key_index,d.name,d.cate_id,d.send_three_msg_time,u.user_name,u.mobile FROM ".DB_PREFIX."ext_deal_repay dl LEFT JOIN ".DB_PREFIX."ext_deal d  ON d.id=dl.deal_id left join ".DB_PREFIX."user u on dl.user_id=u.id WHERE $condition ORDER BY $order $sort  LIMIT ".$limit;
		$list = $GLOBALS['db']->getAll($sql_list);
		foreach($list as $k=>$v){
			$list[$k]['l_key_index'] = "第 ".$v['l_key_index']." 期";
			if($v['send_three_msg_time'] == $v['repay_time']){
	...
	public function q_contract()
	{
		...
		$loan_list = $GLOBALS['db']->getAll("select * FROM ".DB_PREFIX."ext_deal_load WHERE deal_id=".$id." ORDER BY create_time ASC");
