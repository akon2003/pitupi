    ...
    public function do_login() {
		...
		if($adm_data) {//有用户名的用户
			...
			if($adm_data['adm_password']!=md5($adm_password)) {
				...
				//加入授权中心id
				$adm_session['is_authority'] = $adm_data['is_authority'];
				$adm_session['authority_id'] = $adm_data['authority_id'];
	...
	public function checkIdCard(){
		...
		if($class_name){
		    ...
		    if($return['status']==1){
				...
				$card_info = idCardSubInfo($card);
				$reeturn['cardNo'] = $card;
				$reeturn['realname'] = $realname;			
				$reeturn['address'] = "---";
				$reeturn['birthday'] = "{$card_info['bmonth']}月{$card_info['bday']}日";
				$reeturn['sex'] = $card_info['sex']? '男':'女';			
