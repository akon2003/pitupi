...
class OfcAction extends CommonAction{
	...
	public function sale_line()
	{
		...
		for($i=0;$i<=30;$i++)
		{
			...
			{				
				...
				{				
					if($row['money']>$max_value)$max_value = $row['money'];
					$data_row = array("value"=>floatval($row['money']),"tip"=>$stat_time."借款".round($row['money'],2)."元");
				}				
			}
		...
		for($i=0;$i<=30;$i++)
		{
			...
			{				
				...
				{
					if($row['money']>$max_value)$max_value = $row['money'];
					$data_row = array("value"=>floatval($row['money']),"tip"=>$stat_time."投资".round($row['money'],2)."元");
				}
			}
		...
		for($i=0;$i<=30;$i++)
		{
			...
			{				
				...
				{
					if($row['money']>$max_value)$max_value = $row['money'];
					$data_row = array("value"=>floatval($row['money']),"tip"=>$stat_time."提现".round($row['money'],2)."元");
				}
			}
	...
	/*	$result['y_axis'] = array("max"=>floatval($max_value));
		...
		if(C('LICAI_OPEN') == 1)
		{
			...
			for($i=0;$i<=30;$i++)
			{
				...
				{				
					...
					{
						if($row['money']>$max_value)$max_value = $row['money'];
						$data_row = array("value"=>floatval($row['money']),"tip"=>$stat_time."理财".round($row['money'],2)."元");
					}
				}
		...
		if(C('PEIZI_OPEN') == 1)
		{
			...
			$sql = "SELECT borrow_money as money,create_time as stat_time FROM ".DB_PREFIX."peizi_order where status = 6 and create_time > '".$begin_time_date."' and create_time <= '".$end_time_date."'";
			
			$stat_result = $GLOBALS['db']->getAll($sql);		
			
			$peizi_line_values = array();
			for($i=0;$i<=30;$i++)
			{
				$stat_time = to_date($begin_time+$i*24*3600,"Y-m-d");
				$data_row = array("value"=>0,"tip"=>$stat_time."配资0元");
				foreach($stat_result as $row)
				{				
					if(to_date(to_timespan($row['stat_time']),"Y-m-d")==$stat_time)
					{
						if($row['money']>$max_value)$max_value = $row['money'];
						$data_row = array("value"=>floatval($row['money']),"tip"=>$stat_time."配资".round($row['money'],2)."元");
					}
				}
				$peizi_line_values[] = $data_row;
			}
			
			$peizi_line_element = array("type"=>"line","colour"=>PEIZI_COLOR,"text"=>"配资","width"=>2,"values"=>$peizi_line_values);
		}
		
		
		$result['y_axis'] = array("max"=>floatval($max_value));
		
		if(C('LICAI_OPEN') == 1 && C('PEIZI_OPEN') == 1 )
		{
			$result['elements'] = array($sale_line_element,$refund_line_element,$verify_line_element,$peizi_line_element,$licai_line_element);
		}
		elseif(C('LICAI_OPEN') == 1)
		{
			$result['elements'] = array($sale_line_element,$refund_line_element,$verify_line_element,$licai_line_element);
		}
		elseif(C('PEIZI_OPEN') == 1)
		{
			$result['elements'] = array($sale_line_element,$refund_line_element,$verify_line_element,$peizi_line_element);
		}
		else
		{
			$result['elements'] = array($sale_line_element,$refund_line_element,$verify_line_element);
