	...
	foreach ($peizi_order_list as $k => $v) {
		...
		if ($sever_money >= $money){
			...
			if ($allow_send_msg && app_conf("SMS_ON") == 1){
				//通知用户扣费失败
				//尊敬的用户{$notice.user_name},您的{$notice.order_sn}配资单号,{$notice.fee_date}自动扣费失败,请及时备足资金{$notice.rate_money}到帐户余额,否则将会被强制平仓.
				...
				$msg_data['datas'] = serialize(array(
					...
					'order_sn' => $notice['order_sn'],
					'fee_date' => $notice['fee_date'],
					'rate_money' => $notice['rate_money']
				));
				//end
...
function pay_fee($order_id,$trade_money,$rate_money,$site_money,$allow_send_msg = true){
	...
	if($GLOBALS['db']->affected_rows()){
		...
		//计息金额,用于计算rate_money,site_money的基数,按借款金额计算=p2p_peiziorder.borrow_money；如果按实际使用资金计息方式则为=\r\n1、当天新增+	===》利息，佣金\r\n2、当天卖出+	===》利息，佣金\r\n3、当天持有+（不含：当天新增，卖出部分）===》利息
		...
		*/
		...
		//投资推荐人（p_invest_user_id）获得的: 投资人利息收益的n%返利 = rate_money * p2p_peiziorder.invite_invest_interest_rate
		$fee_data['invite_invest_interest_money'] = $fee_data['rate_money'] * $v['invite_invest_interest_rate'];
		//投资推荐人（p_invest_user_id）获得的: 投资人佣金收益的n%返利 = invest_commission_money * p2p_peiziorder.invite_invest_commission_rate
		...
		//借款推荐人（p_user_id）获得的: 平台利息收益的n%返利 = site_money * p2p_peiziorder.invite_borrow_interest_rate
		$fee_data['invite_borrow_interest_money'] = $fee_data['site_money'] * $v['invite_borrow_interest_rate'];
		//借款推荐人（p_user_id）获得的: 平台佣金收益的n%返利 = site_commission_money * p2p_peiziorder.invite_borrow_commission_rate
		...
		if ($allow_send_msg && app_conf("SMS_ON") == 1){
			...
			//尊敬的用户{$notice.user_name},您的{$notice.order_sn}配资单号,{$notice.fee_date}自动扣费{$notice.rate_money}成功,预计下次扣费时间为{$notice.next_fee_date}.
			...
			$msg_data['datas'] = serialize(array(
				...
				'order_sn' => $notice['order_sn'],
				'fee_date' => $notice['fee_date'],
				'next_fee_date' => $notice['next_fee_date']
			));
			//end
 ...
 */
 ...
 /**
  * 配资申请操作描述
  * @param unknown_type $v p2p_peizi_order_op
  * @param unknown_type $type_format (p2p_peizi_order.type 日，周，月)
  * @return string
  */
