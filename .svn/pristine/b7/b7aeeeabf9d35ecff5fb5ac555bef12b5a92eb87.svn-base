...
class IpslogAction extends CommonAction{
	...
	public function trade_export_csv($page = 1)
	{
		...
		if($list)
		{
	    	...
	    	if($page == 1)
	    	{
		    	$content = iconv("utf-8","gbk","编号,标号,借款名称,商户日期,借款金额,保证金,利率,周期值,周期类型,还款方式,姓名,状态");	    		    	
		    	$content = $content . "\n";
	    	}
	...
	public function creditor_export_csv($page = 1)
	{
		...
		if($list)
		{
			...
			if($page == 1)
	    	{
		    	$content = iconv("utf-8","gbk","编号,标号,借款名称,用户名,商户日期,登记方式,债权面额,交易金额,手续费,姓名");	    		    	
		    	$content = $content . "\n";
	    	}
			foreach($list as $k=> $v)
			{
	...
	*/
	...
	public function guarantor_export_csv($page = 1)
	{
		...
		if($list)
		{
	    	...
	    	if($page == 1)
	    	{
		    	$content = iconv("utf-8","gbk","编号,借款名称,用户名,平台账号,商户开户流水号,商户日期,标的号,担保金额,担保保证金,担保收益,担保方类型,担保方证件号码,担保方账户姓名,担保方账户,备注1,备注2,备注3,担保方编号,实际冻结金额,已代偿金额,IPS处理时间,担保状态");	    		    	
		    	$content = $content . "\n";
	    	}
