...
class ConfAction extends CommonAction{
	...
	public function peizi(){
		$conf_res = M("Conf")->where("is_effect = 1 and `name` in ('PEIZI_SCORE_TYPE','PEIZI_SCORE','PEIZI_LOAD_SCORE_TYPE','PEIZI_LOAD_SCORE','PEIZI_MONEY_SAFE','PEIZI_MONEY_SAFE','PEIZI_LAW')")->order("group_id asc,sort asc")->findAll();
		foreach($conf_res as $k=>$v)
		{
			$v['value'] = htmlspecialchars($v['value']);
			if($v['name']=='TEMPLATE')
		...
		$this->assign("conf",$conf);
		$this->assign("main_title","配资基本配置");
		$this->display();
	}
	
	public function update_peizi(){
		$conf_res = M("Conf")->where("is_effect = 1 and `name` in ('PEIZI_SCORE_TYPE','PEIZI_SCORE','PEIZI_LOAD_SCORE_TYPE','PEIZI_LOAD_SCORE','PEIZI_MONEY_SAFE','PEIZI_MONEY_SAFE','PEIZI_LAW')")->findAll();
		foreach($conf_res as $k=>$v)
		{
			conf($v['name'],$_REQUEST[$v['name']]);
