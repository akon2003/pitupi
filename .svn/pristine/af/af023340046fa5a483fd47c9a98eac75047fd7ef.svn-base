...
//		var_dump($list);exit;
		...
		if($list)
		{
			...
			if($page == 1)
	    	$content_hasback_info = iconv("utf-8","gbk","收款人,借款号,借款标题,借款类型,还款本息,投资者罚息收入,投资者付管理费,借款者付管理费,逾期天数,应收时间,实收时间,投资者实收总额,平台收入,状态");
...
//		var_dump($list);exit;
		...
		if($list)
		{
			...
			if($page == 1)
	    	$content_tobe_receivablesinfo = iconv("utf-8","gbk","收款人,借款号,借款标题,借款类型,还款本息,逾期天数,应收时间,状态");
