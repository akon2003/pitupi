...
class uc_learn
{
	public function index(){
		...
		if ($user_id >0){
			...
			$learn_balance = $GLOBALS['db']->getOne("select sum(money) FROM ".DB_PREFIX."learn_send_list WHERE is_use = 0 and user_id='$user_id' and is_recycle = 0 and begin_time < '$now_time' and '$now_time' < end_time  and is_effect = 1 ");
			...
			if($t=="load"){
				...
				$learn_send_list = $GLOBALS['db']->getAll("select lsl.*,lt.type from ".DB_PREFIX."learn_send_list lsl left join ".DB_PREFIX."learn_type lt on lsl.type_id = lt.id where lt.is_effect = 1 and  lsl.user_id = '$user_id' order by id desc  limit ".$limit);	
				$learn_send_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."learn_send_list  where user_id = ".$user_id);	
