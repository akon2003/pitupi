...
interface collocation{	
	 ...
	 * 创建新帐户
	 * @param int $user_id
	 * @param int $user_type 0:普通用户p2p_user.id;1:担保用户p2p_deal_agency.id
	 ...
	 * 用户提现
	 * @param int $user_id
	 * @param int $user_type 0:普通用户p2p_user.id;1:担保用户p2p_user.id
	 ...
	 * @param int $deal_id  标的id
	 * @param string $ref_data 逗号分割的,代偿，代偿还款列表; 债权转让: id; 结算担保收益:金额，如果为0,则取p2p_deal.guarantor_pro_fit_amt ;
	 * @return string
	 */
