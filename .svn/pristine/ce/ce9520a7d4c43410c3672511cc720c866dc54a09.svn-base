	...
	public function update(){
		...
		if (false !== $list) {
			...
			if($data['status']==1){
				...
				$sh_notice['deal_name'] = $deal_name;				//借款名称
				...
				{
					//尊敬的用户{$notice.user_name},您的"{$notice.deal_name}"续约已成功通过,感谢您的关注和支持.
				...
				$sh_notice['time'] = to_date($vo['create_time'],"Y年m月d日 H:i:s");								//提交时间
				$sh_notice['deal_name'] = D("Deal")->where("id=".$vo['deal_id'])->getField("name");				//借款名称
