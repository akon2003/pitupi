...
class ProjectOrderAction extends CommonAction{
	public function index()
	{
		...
		$order_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."project_order where repay_make_time=0 and repay_time>0  ");
		foreach($order_list as $k=>$v){
				...
				if($repay_make_date<=get_gmtime()){
 					$GLOBALS['db']->query("update ".DB_PREFIX."project_order set repay_make_time =  ".get_gmtime()." where id = ".$v['id'] );
				}
 		}
		//列表过滤器，生成查询Map对象
		$map = $this->_search ();
...
//					syn_deal($deal_id);
	...
	{
		$order_info = M("ProjectOrder")->getById(intval($_REQUEST['id']));
	...
	public function refund()
	{
		...
		if($order_info)
		{
			...
			{
				$GLOBALS['db']->query("update ".DB_PREFIX."project_order set is_refund = 1 where id = ".$id." and is_refund = 0");
	...
	//	$limit=((0).",".(10));
		...
		{
			$where.= " and ".DB_PREFIX."project_order.deal_id = ".intval($_REQUEST['deal_id']);
		}
		if(trim($_REQUEST['deal_name'])!='')
		{
			$where.= " and ".DB_PREFIX."project_order.deal_name = ".intval($_REQUEST['deal_name']);
		}
		if(trim($_REQUEST['user_name'])!='')
		{
			$where.= " and ".DB_PREFIX."project_order.user_name = ".intval($_REQUEST['user_name']);
		}
	...
	public function get_pay_list()
	{
		...
		if($deal_id>0){
			...
			$order_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."project_order where repay_make_time=0 and repay_time>0 and deal_id=$deal_id  ");
			
			
		}else{
			$order_list = $GLOBALS['db']->getAll("select dorder.* from ".DB_PREFIX."project_order as dorder left join ".DB_PREFIX."project as d on d.id=dorder.deal_id where d.type=1 and dorder.repay_make_time=0 and dorder.repay_time>0 ");
			//echo "select dorder.* from ".DB_PREFIX."project_order as dorder left join ".DB_PREFIX."project as d on d.id=dorder.deal_id where d.type=1 and dorder.repay_make_time=0 and dorder.repay_time>0 ";exit;
		}
		foreach($order_list as $k=>$v){
				$left_date=intval(app_conf("REPAY_MAKE"))?7:intval(app_conf("REPAY_MAKE"));
				$repay_make_date=$v['repay_time']+$left_date*24*3600;
