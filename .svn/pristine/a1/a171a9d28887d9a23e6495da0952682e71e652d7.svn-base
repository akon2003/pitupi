...
{//类定义开始
    ...
    static function buildImageVerify($length=4,$mode=1,$type='gif',$width=48,$height=22,$verifyName='verify')
    {
		...
		$margin_left = intval($width / 8);
		$margin_top = intval($height / 2);
		$font_size = $height>36? intval($height / 3) : intval($height / 2);
		...
		for($i=0;$i<$length;$i++) {
			...
			$stringColor = imagecolorallocate($im,mt_rand(150,200),mt_rand(100,200),mt_rand(100,200));
			if($width>=89){
				imagestring($im,5,$i*10+25,mt_rand(15,8),$randval{$i}, $stringColor);
				//imagettftext($im, $font_size, $an, $i*18+$margin_left, $margin_top+rand(1,10), $stringColor, $font, $randval{$i});
			}
			else
			{
