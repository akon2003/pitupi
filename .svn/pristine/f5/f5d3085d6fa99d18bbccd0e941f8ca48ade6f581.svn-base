...
class helpModule extends SiteBaseModule {
	public function index() {			
		...
		if (!$GLOBALS['tmpl']->is_cached('page/help_index.html', $cache_id)) {
			...
			if ($id==88 || $id==120 || $id==121 || $id==135) {
				$GLOBALS['tmpl']->assign("agreement_title",$article['title']);
				$GLOBALS['tmpl']->assign("agreement_content",$article['content']);
				$GLOBALS['tmpl']->display("page/agreement.html");
				exit;
			} else if(!$article||$article['type_id']!=1) {
