...
class borrowModule extends SiteBaseModule {
	function __construct(){
		if(in_array(ACTION_NAME,array("index","quota_submit","savedealquota","stepone","steptwo","savedeal"))){
	    	...
	    	if($GLOBALS['user_info']['is_black']==1){
	    		showErr("您当前无权限借款，具体联系网站客服",$is_ajax,url("index"));
	    	}
		}
		parent::__construct();
	}
    ...
    function quota_submit(){
		//检测是否系统维护模式
		$this->forbid_system_repair_mode();
    ...
    function stepone() {
		...
		$peizi_order_ids = $GLOBALS['db']->getOne("SELECT peizi_order_ids FROM ".DB_PREFIX."deal WHERE (is_delete=2 or is_delete=3) AND user_id=".$GLOBALS['user_info']['id']);
		if($peizi_order_ids) {
			app_redirect(url("index","uc_trader#makedeals"));
		}
		...
		$agreement = 88;//app_conf('BORROW_AGREEMENT');
		...
		// 获取借款类型的标记
		$mark = $GLOBALS['db']->getOne("select mark from ".DB_PREFIX."deal_loan_type where id=".$typeid);	
    ...
    function steptwo(){    	
		//检测是否系统维护模式
		$this->forbid_system_repair_mode('',url("borrow","index"));
	...
	function savedeal(){
		...
		*/
    	...
    	if($deal_id > 0){
			...
			$peizi_order_ids = $GLOBALS['db']->getOne("SELECT peizi_order_ids FROM ".DB_PREFIX."deal WHERE id = ".$deal_id);
    		if($peizi_order_ids)
			{
				$GLOBALS['db']->query("update ".DB_PREFIX."peizi_order set deal_id =".$deal_id." where id in(".$peizi_order_ids.") and deal_id = 0");
			}
		}
    	else{
    		if($t=="save"){
    	...
    	if(
			...
			if($ulist){
				...
				if($uuid > 0){
		    		$content = app_conf("SHOP_TITLE")."用户您好，请尽快上传必要信用认证材料（包括身份证认证、工作认证、收入认证、信用报告认证）。另外，多上传一些可选信用认证，有助于您提高借款额度，也有利于出借人更多的了解您的情况，以便让您更快的筹集到所需的资金。请您点击'我要借款'，之后点击相应的审核项目，进入后，可先阅读该项信用认证所需材料及要求，然后按要求上传资料即可。 如果您有任何问题请您拨打客服电话 ".app_conf('SHOP_TEL')." 或给客服邮箱发邮件 ".app_conf("REPLY_ADDRESS")." 我们会及时给您回复。";
					...
					if($deal['user_id']!=$message['user_id']){
						...
						if($msg_conf['sms_asked']==1){
							...
							/*{$notice.shop_title}用户您好，请尽快上传必要信用认证材料（包括身份证认证、工作认证、收入认证、信用报告认证）。另外，多上传一些可选信用认证，有助于您提高借款额度，也有利于出借人更多的了解您的情况，以便让您更快的筹集到所需的资金。请您点击'我要借款'，之后点击相应的审核项目，进入后，可先阅读该项信用认证所需材料及要求，然后按要求上传资料即可。 如果您有任何问题请您拨打客服电话{$notice.shop_tel}或给客服邮箱发邮件{$notice.shop_address}我们会及时给您回复。*/
   ...
   //撤销审核 
    function reback(){
    	...
    	if($GLOBALS['db']->affected_rows()>0){
			$peizi_order_ids = $GLOBALS['db']->getOne("select group_concat(id) as ids from ".DB_PREFIX."peizi_order where deal_id=".$id);
			if($peizi_order_ids!="")
			{
				$GLOBALS['db']->query("UPDATE ".DB_PREFIX."peizi_order SET deal_id=0 WHERE deal_id=".$id);
    			$GLOBALS['db']->query("UPDATE ".DB_PREFIX."deal SET peizi_order_ids='".$peizi_order_ids."' WHERE publish_wait=1 and is_effect=1 and is_delete=2 and id=".$id);
			}
			showSuccess("借款不存在",1);
    	}
    	else{
