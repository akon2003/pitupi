...
function show_ke_image($id,$cnt="")
{
	if($cnt)
	{
		...
		$image_path =APP_ROOT."/public/images/no_pic.gif";
        ...
        <div style='float:left; height:35px; padding-top:1px;'>
		 ...
		 <div style='float:left; height:35px; padding-top:1px;margin-right:10px'>
			 <div class='buttonActive'>
				<div class='buttonContent'>
					<img src='".APP_ROOT."/public/images/del.gif' style='$is_show; margin-left:10px; float:left; border:#ccc solid 1px; width:35px; height:35px; cursor:pointer;' class='keimg_d' rel='$id' title='删除'>
				</div>
			</div>
		</div>
		</span>";
...
function gen_qrcode($str,$size = 5,$img=false)
{
	...
	if(!file_exists($filesave))
	{
		...
		$logo = APP_ROOT_PATH.app_conf("QRCODE_LOGO");  
...
function getCarryFee($money,$user_info){
	...
	*/
	...
	if(count($fee_config)) {
		$valid = false;
		for ($i=0; $i<count($fee_config); $i++) {
			if ($user_info['user_type'] == $fee_config[$i]['user_type']) {
				$valid = true;
				$fee = $fee_config[$i]['fee'];
				$feel_type = $fee_config[$i]['feel_type'];
				break;
			}
		}
		if ($valid == false) {
...
function site_statics(){
	...
	//if($stats==false){
		...
		//累计投资金额
		$load_rs = $GLOBALS['db']->getRow("SELECT sum(dl.money) as total_load,count(*) as load_count FROM ".DB_PREFIX."deal_load dl left join ".DB_PREFIX."deal d on d.id=dl.deal_id where dl.is_repay= 0 and d.deal_status in (1,2,4,5) and d.is_removed<>1");
		$stats['total_load'] = $load_rs['total_load'];
		$stats['total_load'] = $stats['total_load'] + floatval(app_conf("VIRTUAL_MONEY_1"));
		$stats['load_count'] = $load_rs['load_count'];
		$stats['total_load_format'] = format_conf_count(number_format($stats['total_load'],2));
		//成交笔数
		$stats['deal_total_count'] = $GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."deal where deal_status >=4 and is_removed=0");
		...
		$stats['total_interest_money'] = $total_interest_money;
		$stats['total_interest_money_format'] = format_conf_count(number_format($total_interest_money,2));
		
		//累计创造收益
		$stats['total_rate'] = $GLOBALS['db']->getOne("SELECT sum(dl.true_interest_money + dl.impose_money + dl.true_reward_money - dl.true_manage_money - dl.true_manage_interest_money) FROM ".DB_PREFIX."deal_load_repay dl left join ".DB_PREFIX."deal d on d.id=dl.deal_id where dl.has_repay = 1 and d.is_removed=0");
		...
		$stats['total_rate'] = $stats['total_rate'] + floatval(app_conf("VIRTUAL_MONEY_2"));
		...
		$stats['user_count'] = $stats['user_count'] + intval(app_conf("VIRTUAL_USER_1"));
		//XHF 2016/9/29
		$stats['user_count'] = 4624 + intval(($stats['user_count'] - 4624)/3);
		
        //已还本息总额
		$stats['total_repay_money'] = $GLOBALS['db']->getOne("SELECT sum(dr.true_repay_money) FROM ".DB_PREFIX."deal_repay dr left join ".DB_PREFIX."deal d on dr.deal_id=d.id where dr.has_repay=1 and d.is_removed<>1");
        $stats['total_repay_money_format'] = format_conf_count(number_format($stats['total_repay_money'],2));
