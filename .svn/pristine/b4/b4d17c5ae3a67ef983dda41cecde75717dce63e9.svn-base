...
jQuery(function(){
	...
	$("#Jcarry_bank_id").change(function(){
		if($(this).val()=="other"){
			...
			if($(this).find("option:selected").attr("day")!=undefined)
				...
				$("#Jcarry_bankSuggestNote").html("提现时间为T+1个工作日。");
		}
	});
	...
	$("#Jcarry_From").submit(function(){
		var carry_amount = parseFloat($("#Jcarry_amount").val());
		if(user_type == 0 && carry_amount > 200000) {
			$("#Jcarry_balance").html("提现金额超过银行限制");
		}
		else if(user_type == 1 && carry_amount > 2000000) {
			$("#Jcarry_balance").html("提现金额超过银行限制");
		}
...
function setCarryResult(){
	...
	else if(user_type == 0 && carry_amount > 200000) {
		$("#Jcarry_balance").html("提现金额超过银行限制");
	}
	else if(user_type == 1 && carry_amount > 2000000) {
		$("#Jcarry_balance").html("提现金额超过银行限制");
