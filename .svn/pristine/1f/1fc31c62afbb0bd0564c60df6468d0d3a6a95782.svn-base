	...
	public function update(){
		...
		if ($list >0) {
			...
			if($data['status']==1){
				...
				{
					//尊敬的用户{$notice.user_name},您的{$notice.quota_money}元信用额度申请已成功,感谢您的关注和支持.
					...
					$msg_data['datas'] = serialize(array(
						...
						'quota_money' => $notice['quota_money']
					));
					//end
				...
				if(app_conf("SMS_ON")==1)
				{
					//尊敬的用户{$notice.user_name},您的{$notice.quota_money}元信用额度申请因"{$notice.msg}"审核失败了.
