...
class register_baidu{
	public function index()
	{
		$user_name = strim($GLOBALS['request']['mobile']);//用户名
		...
		if(isset($GLOBALS['request']['r'])){
			$wap_referer = strim(base64_decode($GLOBALS['request']['r']));
			$root['wap_referer'] = $wap_referer;
		}
		$root['program_title'] = "注册页面";
		...
		$user_data = array(
			...
			'user_pwd'         => $user_pwd,		
			'referer'		   => $referer,
		    'pid'			   => intval($GLOBALS['request']['rel_uid'])
		);
