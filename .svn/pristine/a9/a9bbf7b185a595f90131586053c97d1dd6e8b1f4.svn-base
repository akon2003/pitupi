...
class uc_vip_buy_log
{
	public function index(){
		...
		if ($user_id >0){
	    	...
	    	$vip_buy_log_list = $GLOBALS['db']->getAll("select vbl.*,vt.vip_grade from ".DB_PREFIX."vip_buy_log vbl LEFT JOIN ".DB_PREFIX."vip_type vt ON vbl.vip_id=vt.id  where vbl.user_id = ".$user_id." order by vbl.id desc limit $limit ");
			$log_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."vip_buy_log  where user_id = ".$user_id." ");
