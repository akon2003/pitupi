...
$(document).ready(function(){
	...
	//添加单击选中事件
	$(".dataTableSelect .row").bind("click dblclick",function(){
		if ($(this).hasClass("selected")) {
			...
			if ($(this).find("input:checkbox")) {
				...
				$(this).siblings().find("input:checkbox").attr("checked","");
			}
		}
	});
	//添加单击多行选中
	$(".dataTableSelectMulti .row").bind("click",function(){
		if ($(this).hasClass("selected")) {
			$(this).removeClass("selected");
			if ($(this).find("input:checkbox")) {
				$(this).find("input:checkbox").attr("checked",false);
	...
	//添加双击编辑事件
	$(".dataTableSelectDblclick .row").bind("dblclick",function(){
		if (!$(this).hasClass("selected")) {
			$(this).addClass("selected").siblings().removeClass("selected");
			if ($(this).find("input:checkbox")) {
				$(this).find("input:checkbox").attr("checked",true);
...
function del(id)
{
	...
	if($.showCfm(LANG['CONFIRM_DELETE']))
	$.ajax({ 
			url: ROOT+"?"+VAR_MODULE+"="+MODULE_NAME+"&"+VAR_ACTION+"=delete&id="+id, 
			data: "ajax=1",
			dataType: "json",
			success: function(obj){alert("OK");
				$("#info").html(obj.info);
				if(obj.status==1)
...
function export_csv()
{
	...
	{
		param += "&"+radios[i].name+"="+$(radios[i]).val();
	}
...
//XHF 2016/7/28 添加input:radio的判断
function export_csv_ext(action)
{
	var inputs = $(".search_row").find("input:[type!='chekcbox']");
	var checkboxs = $(".search_row").find("input:[type='checkbox']:checked");
	var radios = $(".search_row").find("input:[type='radio']:checked");
	var selects = $(".search_row").find("select");
	var param = '';
	...
	var url= ROOT+"?"+VAR_MODULE+"="+MODULE_NAME+"&"+VAR_ACTION+"="+action;
	location.href = url+param;
}
