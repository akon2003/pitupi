...
class uplan_list
{
    public function index(){
        ...
        // Admin 2016-7-16
        // 显示多条U计划,包含已满标的
        $uplan_3 = get_deal_list('0,1',0,' publish_wait=0 and is_new=0 and repay_time=3 and deal_status in (1,2,4,5) ','id desc','','',false,'uplan');
        $uplan_6 = get_deal_list('0,1',0,' publish_wait=0 and is_new=0 and repay_time=6 and deal_status in (1,2,4,5) ','id desc','','',false,'uplan');
        $uplan_12 = get_deal_list('0,1',0,' publish_wait=0 and is_new=0 and repay_time=12 and deal_status in (1,2,4,5) ','id desc','','',false,'uplan');
        // end
        $root = array();
        $root['response_code'] = 1;
        $root['item'] = $result['list'];
        $root['uplan'] = array_merge($uplan_3['list'],$uplan_6['list'],$uplan_12['list']);
        //$root['DEAL_PAGE_SIZE'] = app_conf("DEAL_PAGE_SIZE");
        //$root['count'] = $result['count'];
        $root['page'] = array("page"=>$page,"page_total"=>ceil($uplan['count']/app_conf("DEAL_PAGE_SIZE")),"page_size"=>app_conf("DEAL_PAGE_SIZE"));
        $root['program_title'] = "U计划";
        output($root);
    }
}
?>
