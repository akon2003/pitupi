...
class ajaxModule extends SiteBaseModule
{
	public function __construct(){
		parent::__construct();
		$no_action_array = array("check_field","auto_charging_rate_money","load_api_url","weixin_login");
	...
	public function get_email_verifys() {
		...
		*/
		$data = send_user_verify_mails($email,$code,$GLOBALS['user_info']);
		if ($data['status']) {
			//$data['info'] = "验证邮件已经发送，请注意查收";
			$data['info'] = "验证邮件已经发送，请注意查收".$data['msg'];
		} else {
			//$data['info'] = "验证邮件发送失败";
			$data['info'] = "验证邮件发送失败".$data['msg'];
		}
		ajax_return($data);
		// 结束
	}
	...
	public function get_register_verify_code() {
		...
		//XHF 2016/8/7 补全参数
		send_verify_sms($user_mobile,$verify_data['verify_code'],null,true);
		//end
	...
	public function get_re_pwd_verify_code() {
		...
		ajax_return($data);
		*/
		$data = send_verify_sms($user_mobile,$verify_data['verify_code'],$users,true);
	 ...
	 * 不依赖于验证码开关设置
	 */
	public function get_check_verify()
	{
		$verify = isset($_REQUEST['verify'])? $_REQUEST['verify'] : "";
		$result['status'] = 0;

		if ($verify != "") {
			$verify = md5(trim($verify));
			$session_verify = es_session::get('verify');
			if($verify != $session_verify) {		
				$result['status'] = 0; 
			} else {
