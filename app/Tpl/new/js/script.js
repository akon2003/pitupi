...
function bindKeUpload(){
	...
	if ($(".keimg").length > 0) {
	    ...
	    K('.keimg_d').click(function() {
	        ...
	        dom.find("#keimg_m_"+node.attr("rel")).attr("src",ROOT_PATH + "/cache/Tpl/default/Common/images/no_pic.gif");
...
function sendPhoneCode0(o, obj){
	if (!$(o).hasClass('btn_disable')) {
		$(o).addClass('btn_disable');
		get_verify_code(obj,APP_ROOT+"/index.php?ctl=ajax&act=get_paypwd_verify_code",function(){
...
function openNewWindow(src,stitle,swidth,sheight){
	if(sheight >= $(window).height()){
		sheight = $(window).height() - 120;
	}
	window.open(src,stitle,'height='+sheight+',width='+swidth+'top=0,left=0,toolbar=no,menubar=no,scrollbars=no,resizable=no,location=no,status=no');
}
