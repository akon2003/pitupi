<?php
    ...
    /**
     * Admin 2016-7-14
     * function getUcRepayPlan
     * 返回数据中添加deal.ext判断正常标/U计划/新手标
     * Admin 2016-7-14
     * function getUcDealRepay
     * 返回数据中添加deal.ext判断正常标/U计划/新手标
     */
...
//					}
	...
	function getInchargeDone($payment_id,$money,$bank_id,$memo,$pingzheng,$debit_id = 0,$debit_type = 0)
	{
		...
		// 是否有相同的充值
        // 关闭相同金额限制 Admin 2016-7-13
        /*
		$list=$GLOBALS['db']->getRow("select * from ".DB_PREFIX."payment_notice where user_id=".$GLOBALS['user_info']['id']." and money = ".floatval($money)." and create_time > ".(TIME_UTC-60*60));
		if ($list) {
			$status['status'] = 0;
			$status['show_err'] = "为保障帐户资金安全，一小时内请勿重复提交相同金额";
			return $status;
		}
        */
        // end
	...
	function getUcSaveCarry($amount,$paypassword,$bid){
		...
		if($GLOBALS['user_info']['id'] > 0){
			...
			if ($GLOBALS['user_info']['user_type'] == 0 && $data['money'] > 200000) {
				$status['status'] = 0;
				$status['show_err'] = '账户提现金额超过200,000元';
				return $status;
			} else if ($data['money'] > 2000000) {
				$status['status'] = 0;
				$status['show_err'] = '账户提现金额超过2,000,000元';
				return $status;
			}

			// 是否有相同的提现
			$list=$GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_carry where user_id=".$data['user_id']." and money = ".floatval($amount)." and create_time > ".(TIME_UTC-60*60));
			if ($list) {
				$status['status'] = 0;
				$status['show_err'] = '为保障帐户资金安全，一小时内请勿重复提交相同金额';
				return $status;
			...
			if((floatval($fee) + floatval($data['money'])) <= get_user_info("AES_DECRYPT(money_encrypt,'".AES_DECRYPT_KEY."')-nmc_amount","id=".intval($GLOBALS['user_info']['id']),"ONE")){
				...
				//$content = "您于".to_date($data['create_time'],"Y年m月d日 H:i:s")."提交的".format_price($data['money'])."提现申请我们正在处理，如您填写的账户信息正确无误，您的资金将会于T+1个工作日内到达您的银行账户.";
	...
	function getUcRepayPlan($user_id,$status,$limit,$condition=""){
		...
		if($result['rs_count'] > 0){
			...
			*/
            $sql_list = "SELECT dlr.*,dlr.l_key +1 as l_key_index ,d.name,d.ext FROM ".DB_PREFIX."deal_load_repay dlr LEFT JOIN ".DB_PREFIX."deal d On d.id = dlr.deal_id where $extWhere $condition ORDER BY dlr.repay_time DESC LIMIT ".$limit;
            // end
			...
			foreach($result['list'] as $k=>$v){
				...
				$load_money = $GLOBALS['db']->getOne("select money from ".DB_PREFIX."deal_load where user_id=".$result['list'][$k]['user_id']." and deal_id=".$result['list'][$k]['deal_id']);
				$result['list'][$k]['load_money_format'] = format_price($load_money);
			}
		}
		return $result;
	}
	...
	function getUcDealRepay($user_id,$limit,$condition=""){
		...
		$result['rs_count'] = $GLOBALS['db']->getOne($sql_count);
		if($result['rs_count'] > 0){
			$result['list']=$GLOBALS['db']->getAll("select *,l_key+1 as l_key_index from ".DB_PREFIX."deal_repay where $extWhere $condition order by repay_time asc limit ".$limit);
			foreach($result['list'] as $k=>$v){
				$result['list'][$k]['name']= $GLOBALS['db']->getOne("select name from ".DB_PREFIX."deal where id = ".$result['list'][$k]['deal_id']);//借款名称
				$result['list'][$k]['l_key_index'] = "第 ".$v['l_key_index']." 期";
				if($v['has_repay'] == 0){
					$result['list'][$k]['status_format'] = '待还';
				}elseif($v['status'] == 0){
                ...
                // Admin 2016-7-14
                // 获取 d.ext用于url格式化
				$result['list'][$k]['ext']= $GLOBALS['db']->getOne("select ext from ".DB_PREFIX."deal where id = ".$result['list'][$k]['deal_id']);
                // end
			}
		}
		return $result;
