 ...
 * 获取正在进行的投标列表
 */
function get_deal_list($limit="",$cate_id=0, $where='',$orderby = '',$user_name='',$user_pwd='',$is_all=false,$ext=-1)
{
	$time = TIME_UTC;
    ...
    if ($ext == -1) {
        ...
        $extNameW = " and ext='".$ext."'";
    }

	$count_sql = "select count(*) from ".DB_PREFIX."deal where 1=1 ".$extNameW;
	if($is_all==false)
	...
	$sql = "select *,start_time as last_time,(load_money/borrow_amount*100) as progress_point,(start_time + enddate*24*3600 - ".$time.") as remain_time $extfield from ".DB_PREFIX."deal where 1 = 1 ".$extNameW;
	if($is_all==false)
...
function format_deal_item(&$deal,$user_name="",$user_pwd=""){
	...
	//可投资金额
	$deal['remain_load_money_format'] = format_price(($deal['borrow_amount'] - $deal['load_money'])/10000)."万";
	...
	if($deal['deal_status']>=4){
		...
		if($deal['repay_time_type'] == 0) {
            ...
            $deal["end_repay_time"] = to_next_month($deal['repay_start_time'], $deal['repay_time']);
        }

        if ($deal["repay_start_time"] > 0) {
            $deal["end_repay_time_format"] = date('Y-m-d',$deal["end_repay_time"]);
        } else {
            $deal["end_repay_time_format"] = "";
        }
    ...
    if ($deal['ext'] != '') {
        $durl = url("index",$deal['ext'],array("id"=>$deal['id']));
    } else {
	...
	// Admin添加处理显示用于进度条显示
	// 状态为等待和进行中的才显示进度
	if ($deal['deal_status'] == 0 || $deal['deal_status'] == 1) {
		if ($deal['progress_point'] > 100) { $deal['progress_point'] = 100; }	
		// 移除小数位
		$deal['progress_point'] = intval($deal['progress_point']);
	} else {
		// 满标|还款中|已还清都修改为满标状态
		$deal['progress_point'] = 100;
	}
	$deal['progress_point_len'] = $deal['progress_point'] * 4;
	// 添加募集期
	$deal['start_to_end'] = date('Y/m/d',$deal['start_time']).' - '.date('Y/m/d',$deal['start_time']+($deal['enddate']*24*3600));
	// 上次还款日期
	$deal['last_repay_time'] = $deal['last_repay_time']? date('m月d日',$deal['last_repay_time']) : '';
 ...
 *  	
 *  支付给“推荐人”的 [“借款者”管理费返利 ] p2p_deal_repay.manage_money_rebate 需要本期所有的还完后，才计算
 */
...
function check_dobid2($deal_id,$bid_money,$bid_paypassword,$is_pc = 0,$ecv_id=0){	
	...
	if(($deal['is_new']==1 || $deal['ext'] == 'newe') &&  $GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."deal_load  WHERE user_id=".$GLOBALS['user_info']['id']." ") > 0){
		$root["show_err"] = "此标为新手专享标，您已经投过借款了";
		return $root;
	}
...
function dobid2_ok($deal_id,$user_id){
	...
	if($deal['deal_status']==1 && $deal['progress_point'] >= 50 && $deal['progress_point']<=60 && $deal['is_send_half_msg'] == 0)
	{
		...
		if(app_conf("MAIL_ON") && $user_info['email'] != ""){
			if(!$msg_conf || intval($msg_conf['mail_half'])==1){
				$tmpl = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."msg_template where name = 'TPL_DEAL_HALF_EMAIL'",false);
...
function dobid2($deal_id,$bid_money,$bid_paypassword,$is_pc=0,$ecv_id=0){
	...
	if($load_id > 0){
		...
		if(app_conf('SMS_ON')==1){
			//发送投标短信
			//尊敬的用户${sh_notice.user_name}，您于${sh_notice.time}投标了借款“${sh_notice.deal_name}”，冻结资金${sh_notice.money}元。
			$tmpl = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."msg_template where name = 'TPL_SMS_DEAL_LOAD'",false);
			$tmpl_content = $tmpl['content'];
...
function getUCInrepayRepayBorrowMoney($id,$user_id = 0){
	...
	foreach($user_loan_list as $lllk=>$lllv){//循环用户
		...
		if($user_repay_money >0 || $user_impose_money >0 || $user_manage_money > 0 || $user_manage_interest_money >0 || $user_reward_money > 0){
			...
			if($user_manage_interest_money>0)
			...
			if(app_conf("SMS_ON")==1&&app_conf('SMS_REPAY_TOUSER_ON')==1){
				...
				$notice['deal_name'] = $deal['sub_name'] != ""? $deal['sub_name'] : $deal['name'];
