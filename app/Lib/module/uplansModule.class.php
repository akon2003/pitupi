 ...
 * XHF 2016-7-10
 * 新增控制器,U计划
 * deal表 ext 字段名为 uplan
 */
require APP_ROOT_PATH.'app/Lib/deal.php';
require 'dealsModule.class.php';
class uplansModule extends dealsModule
{
	public function index(){
		...
		if (!$GLOBALS['tmpl']->is_cached("page/uplans.html", $cache_id))
		{	
			...
			{		
				...
				$v['url'] = url("index","uplans",array("cid"=>$v['id']));
				$deal_cates[] = $v;
			}
			unset($deal_cates_db);
			...
			if($cate_id > 0){
				...
				$total_money = $GLOBALS['db']->getOne("SELECT sum(borrow_amount) FROM ".DB_PREFIX."deal WHERE cate_id=$cate_id AND deal_status in(4,5) AND is_effect = 1 and is_delete = 0 AND ext='uplan'");
			}
			elseif ($cate_id == 0){
				$n_cate_id = 0;
				if($field && $field_sort)
					...
					$orderby = "sort DESC , id DESC";
				$total_money = $GLOBALS['db']->getOne("SELECT sum(borrow_amount) FROM ".DB_PREFIX."deal WHERE deal_status in(4,5) AND is_effect = 1 and is_delete = 0 and ext='uplan'");
			}
			elseif ($cate_id == "-1"){
				$n_cate_id = 0;
				$condition .= "AND deal_status in(2,4,5) AND ext='uplan'";
			...
			$result = get_deal_list($limit,$n_cate_id,$condition,$orderby,'','',false,'uplan');
			...
			$tmp_args = $page_args;
			$tmp_args['cid'] = 0;
			$cate_list_url[0]['url'] = url("index","uplans#index",$tmp_args);
			...
			foreach($deal_cates as $k=>$v){
				...
				$tmp_args = $page_args;
				$tmp_args['cid'] = $v['id'];
				$cate_list_url[$k+1]['url'] = url("index","uplans#index",$tmp_args);
			}
			...
			foreach($interest_url as $k=>$v){
				...
				$interest_url[$k]['url'] = url("index","uplans#index",$tmp_args);
			}
			...
			foreach($months_type_url as $k=>$v){
				...
				$months_type_url[$k]['url'] = url("index","uplans#index",$tmp_args);
			}
			...
			foreach($deal_status_url as $k=>$v){
				...
				$deal_status_url[$k]['url'] = url("index","uplans#index",$tmp_args);
			}
			...
			$tmp_args = $page_args;
			$tmp_args['level'] = 0;
			$level_list_url[0]['url'] = url("index","uplans#index",$tmp_args);
			$level_list_url[0]['name'] = "不限";
			foreach($level_list['list'] as $k=>$v){
				$tmp_args = $page_args;
				$tmp_args['level'] = $v['id'];
				$level_list_url[$k+1] = $v;
				$level_list_url[$k+1]['url'] = url("index","uplans#index",$tmp_args);
			}
			...
			//标状态
			$loantype_url = array();
			$loantypes= $GLOBALS['db']->getAll("SELECT distinct loantype FROM ".DB_PREFIX."deal where ext='uplan' ORDER BY loantype ASC ",1);
			$loantype_url[0]['url'] = url("index","uplans#index",$tmp_args);
			...
			foreach($loantypes as $k=>$v){
				...
				$loantype_url[$v['loantype']+1]['url'] = url("index","uplans#index",$tmp_args);
			}
            ...
            if($temp_city_urls){
                ...
                $tmp_args = $page_args;
                $tmp_args['city'] = 0;
                $city_urls[0]['url'] = url("index","uplans#index",$tmp_args);
                ...
                foreach($temp_city_urls as $k=>$v){
                    if(isset($v['id'])){
                        ...
                        $tmp_args = $page_args;
                        $tmp_args['city'] = $v['id'];
                        $city_urls[$v['id']]['url'] = url("index","uplans#index",$tmp_args);
                    }
                }
            }
			...
			if($sub_citys){
				foreach($sub_citys as $k=>$v){
					...
					$tmp_args['city'] = $v['pid'];
					$tmp_args['scity'] = $v['id'];
					$sub_citys[$k]['url'] = url("index","uplans#index",$tmp_args);
				}
			}
            ...
            foreach($user_type_urls as $k=>$v){
                ...
                $user_type_urls[$k]['url'] = url("index","uplans#index",$tmp_args);
            }
		...
		$GLOBALS['tmpl']->display("page/uplans.html",$cache_id);
	}
