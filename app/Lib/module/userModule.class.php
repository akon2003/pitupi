...
class userModule extends SiteBaseModule {
	...
	public function doregister() {
		...
		*/
		...
		if(isset($user_data['referer']) && $user_data['referer']!=""){
			$p_user_data = get_user_info("id,user_type,authority_id","mobile_encrypt =AES_ENCRYPT('".$user_data['referer']."','".AES_DECRYPT_KEY."') OR user_name='".$user_data['referer']."'");
			...
			if ($p_user_data["user_type"] == 0) {
				//加入授权中心
				$user_data['authority_id'] = $p_user_data["authority_id"];
			}
		}
		...
		{
			$user_data['user_name'] = $user_data['mobile'];
		}
		// 结束
	...
	public function dologin() {
		...
		if($result['status']) {	
			...
			if($result['data'] == ACCOUNT_PASSWORD_ERROR) {
				...
				//加入密码错误次数
				$err = $GLOBALS['lang']['PASSWORD_ERROR'].'，累计次数('.$result['times'].'/5)';
			}
	...
	public function do_re_name_id()
	{
		...
		// 修改 XHF
		// 修改日期 <2016-03-25> 开始
		// 代码修改,添加实名认证在线认证,个人信息从身份证获取
		$verify = idCardVerify($real_name, $idno);

		if($verify['status'] == 0){
			showErr(/*$verify['info']*/"认证请求失败,请稍后再试!");
		}
		if($verify['isok'] == false){
			showErr(/*$verify['info']*/"身份证跟姓名不匹配");
		}

		$id_card_info = idCardSubInfo($idno);
		if ($id_card_info['sex'] != $sex) {
			showErr("性别与身份证识别的性别不一致");
		}			
		// 结束
			...
			*/
			...
			if($res['status'] == 1)
			{
				...
				*/
				...
				require_once APP_ROOT_PATH."app/Lib/module/creditModule.class.php";
				$credit = new creditModule();
				if (!$credit->modify_certification_passed()) {
					showErr("系统自动审核添加失败，请联系客服");
				}
				// 结束
	...
	{
		$img = url("index","ajax#weixin_login",array("rand"=>TIME_UTC));
