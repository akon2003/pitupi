...
class dealsModule extends SiteBaseModule
{
	public function index(){
		...
		{	
			...
			//XHF 2016/8/27 加入U计划显示在列表中
			/*
			$result = get_deal_list($limit,$n_cate_id,$condition,$orderby,'','',false,'');

			//XHF 2016/7/29 添加在投的U计划
			//同时将U计划加入到列表最前端
			$online_uplan_condition = " 1=1 and is_new=0 and is_delete=0 and is_effect=1 and ext='uplan' and deal_status in (1)"; 
			$online_uplan_orderby = " repay_time desc"; 
			$online_uplans = get_deal_list('',0,$online_uplan_condition,$online_uplan_orderby,'','',false,'uplan');
			$result['list'] = array_merge($online_uplans['list'], $result['list']);
			//end 2016/7/29
			*/
			$condition .= " AND (ext='' or ext='uplan') ";
			$orderby = " deal_status ASC,start_time DESC ";
			$result = get_deal_list($limit,$n_cate_id,$condition,$orderby,'','',false);
			//end
