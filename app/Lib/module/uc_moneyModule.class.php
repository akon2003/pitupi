...
class uc_moneyModule extends SiteBaseModule {
	...
	public function incharge() {
		...
		$GLOBALS['tmpl']->assign("idcardpassed",$GLOBALS['user_info']['idcardpassed']);
		...
		foreach($payment_list as $k=>$v) {
			...
			if(file_exists($file)) {
				...
				// 修改页面显示代码中默认选中通联支付接口方式
				$display_code = $payment_object->get_display_code();
				$display_code = preg_replace("/type=\'radio\'/i", "type='radio' checked", $display_code);
				$payment_list[$k]['display_code'] = $display_code;				 		
			} else {
				unset($payment_list[$k]);
			}
		}
	...
	public function incharge_done() {
		...
		if ($GLOBALS['user_info']['idcardpassed'] == 0) {
			showErr("您还没有完成实名认证, 请实名认证后充值!",0,"index.php?ctl=uc_account&act=security");
		}
	...
	public function addbank(){
		//检测是否系统维护模式
		$this->forbid_system_repair_mode(true,url("index","uc_money#bank"));
		...
		$bank_list = $GLOBALS['db']->getAll("SELECT * FROM ".DB_PREFIX."bank ORDER BY sort DESC,id ASC");
	...
	public function checkbank() {
		require_once (APP_ROOT_PATH.'app/Lib/bankList.php');
		$bankcard = trim($_REQUEST['bankcard']);

		$data = array('status'=>1, 'info'=>'');
		if (isset($bankList[substr($bankcard, 0, 8)])) 
		{
			$data['info'] = $bankList[substr($bankcard, 0, 8)];
		} 
		else if (isset($bankList[substr($bankcard, 0, 6)])) 
		{
			$data['info'] = $bankList[substr($bankcard, 0, 6)];
		} 
		else if (isset($bankList[substr($bankcard, 0, 5)])) 
		{
			$data['info'] = $bankList[substr($bankcard, 0, 5)];
		} 
		else if (isset($bankList[substr($bankcard, 0, 4)])) 
		{
			$data['info'] = $bankList[substr($bankcard, 0, 4)];
		}
		else 
		{
			$data['status'] = 0;
			$data['info'] = '该卡号参考信息暂未录入';
		}

		if($GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."user_bank WHERE bankcard='".$bankcard."'") > 0){
			$data['status'] = -1;
			$data['info'] = '该银行卡已存在';
		}
	...
	public function savebank(){
		...
		require_once (APP_ROOT_PATH.'app/Lib/bankList.php');
		$bank_info = '';
		if (isset($bankList[substr($data['bank_id'], 0, 8)])) 
		{
			$bank_info = $bankList[substr($data['bank_id'], 0, 8)];
		} 
		else if (isset($bankList[substr($data['bank_id'], 0, 6)])) 
		{
			$bank_info = $bankList[substr($data['bank_id'], 0, 6)];
		} 
		else if (isset($bankList[substr($data['bank_id'], 0, 5)])) 
		{
			$bank_info = $bankList[substr($data['bank_id'], 0, 5)];
		} 
		else if (isset($bankList[substr($data['bank_id'], 0, 4)])) 
		{
			$bank_info = $bankList[substr($data['bank_id'], 0, 4)];
		}
		
		if (preg_match('/信用卡/',$bank_info)) {
			showErr("不支持信用卡",1);
		}
	...
	public function carry() {
		...
		$GLOBALS['tmpl']->assign("user_type",$GLOBALS['user_info']['user_type']);
		$GLOBALS['tmpl']->assign("fee_config",$fee_config);
		$GLOBALS['tmpl']->assign("json_fee",json_encode($json_fee));
		unset($fee_config);
		unset($json_fee);
