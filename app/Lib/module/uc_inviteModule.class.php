...
class uc_inviteModule extends SiteBaseModule
{
	...
	function invite(){
		...
		*/
		...
		$count = $GLOBALS['db']->getOne("select count(id) from ".DB_PREFIX."user where pid=".$GLOBALS['user_info']['id']);
		$result = $GLOBALS['db']->getAll("select id,user_name,real_name,mobile,create_time from ".DB_PREFIX."user where pid=".$GLOBALS['user_info']['id']." order by id desc limit ".$limit);
