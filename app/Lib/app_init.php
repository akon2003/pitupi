...
if ($user_info && $user_info['id']>0) {
	require_once APP_ROOT_PATH."system/libs/user.php";
	$state = login_session_state($user_info['id'],es_session::id());
	if ($state['status'] == -1) {
		es_session::set('user_info',array());
		echo "<script language='javascript'>alert('账号在其他地方登录,您已被迫下线,如果密码泄露,请及时修改!');location.href='/index.php';</script>";
		exit;
	} else if ($state['status'] == 2) {
		echo "<script language='javascript'>alert('账号在其他地方登录,如果密码泄露,请及时修改!');location.reload();</script>";
	}
}
