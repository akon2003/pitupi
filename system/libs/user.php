...
define("ACCOUNT_NO_VERIFY_ERROR",3); //帐户未激活
	 ...
	 * @param $logout  退出系统
	 * @param $type  返回类型 例如json
	 * 返回：数组格式 status状态
	 * status:  状态 0未登录 1唯一登录 2账号曾在其他地方登录或非正常退出系统,覆盖登录 -1账号已被覆盖
	 * last_login_time: 上次登录时间,-1为覆盖登录时间
	 * last_login_ip: 上次登录IP,-1为覆盖登录IP
	 * Admin 2016/7/29
	 */
	function login_session_state($user_id,$session_id,$logout=false,$type='')
	{
		//用户退出系统
		if ($logout == true) {
			$GLOBALS['db']->query("update ".DB_PREFIX."session_ext set is_online=0 where user_id=".$user_id);
			return true;
		}
		...
		//当前登录的登录信息
		$current_session = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."session_ext where user_id=".$user_id." and session_id='".$session_id."'");
		if (!$current_session) {
			//未记录用户的登录信息,在这里加入记录
			$login_ip = $_SERVER["REMOTE_ADDR"];
			$session_time = time();
			$GLOBALS['db']->query("insert into ".DB_PREFIX."session_ext (is_online,user_id,session_id,session_time,login_ip) values(1,".$user_id.",'".$session_id."',".time().",'".$login_ip."')");
			$current_session = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."session_ext where user_id=".$user_id." and session_id='".$session_id."'");
		}
		$status['current_login_time'] = $current_session['session_time'];
		$status['current_login_ip'] = $current_session['login_ip'];
		

		if ($current_session['is_online'] == 0) {
			//已被覆盖
			$status['status'] = -1;
			//获取最新的登录信息
			$latest_session = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."session_ext where user_id=".$user_id." and is_online=1 and id>".$current_session['id']." order by id desc limit 0,1");
			if ($latest_session) {
				$status['latest_login_time'] = $latest_session['session_time'];
				$status['latest_login_ip'] = $latest_session['login_ip'];
			} else {
				$status['status'] = 1;
			}
		} else {
			//是否曾在其他地方登录或非正常退出系统
			$last_session = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."session_ext where user_id=".$user_id." and is_online=1 and id <".$current_session['id']." order by id desc limit 0,1");
			if ($last_session) {
				$status['status'] = 2;
				//清除登录状态
				$GLOBALS['db']->query("update ".DB_PREFIX."session_ext set is_online=0 where user_id=".$user_id." and id <".$current_session['id']);
				$status['last_login_time'] = $last_session['session_time'];
				$status['last_login_ip'] = $last_session['login_ip'];
			} else {
				//正常登录
				$status['status'] = 1;
				//获取上一次登录信息
				$last_session = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."session_ext where user_id=".$user_id." and id<".$current_session['id']." order by id desc limit 0,1");
				$status['last_login_time'] = $last_session['session_time'];
				$status['last_login_ip'] = $last_session['login_ip'];
			}
		}
		...
		if ($status['status'] == 1 || $status['status'] == 2) {
			$GLOBALS['db']->query("update ".DB_PREFIX."session_ext set session_time=".time()." where user_id=".$user_id." and session_id = '".$session_id."'");
		}
			...
			if(isset($data['money']) || isset($data['fee_amount'])){
				...
				switch((int)$type){ 
					...
					case 1 : //充值手续费 +
						...
						break;
					//case 9 : //提现手续费 + Admin 2016/8/1 关闭提现手续费
					case 10 : //借款管理费(风险保障金) +
					case 12 : //逾期管理费 +
					//case 13 : //人工充值
					case 14 : //借款服务费 +
					case 17 : //债权转让管理费  +
					//case 18 : //开户奖励   -
					//case 20 : //投标管理费 + Admin 2016/8/2
					//case 22 : //兑换  Admin 2016/8/2
					//case 23 : //邀请返利 - Admin 2016/8/2
					//case 24 : //投标返利 - Admin 2016/8/2
					//case 25 : //签到成功 - Admin 2016/8/2
					case 26 : //逾期罚金（垫付后）
					//case 27 : //其他费用
					//case 28 : //投资奖励 Admin 2016/8/2
					//case 29 : //红包奖励 Admin 2016/8/2
					//case 47 : //体验金收益 Admin 2016/8/2
					case 52 : //尽职调查费 +
					case 53 : //其他收益 +
						$is_add = true;
	...
	function do_login_user($user_name_or_email,$user_pwd)
	{
        //Admin 2016-7-31
        //用户名与密码不能为空
        if(trim($user_name_or_email) == "" || trim($user_pwd) == "")
		{			
			$result['status'] = 0;
		...
		//查询用户账号是否异常锁定(密码超过5次错误锁定)
		//Admin 2016/7/28
		if ($user_data) {
			if ($user_data['sohu_id'] >= 5) {
				$result['status'] = 0;
				$result['data'] = -1;

				// Admin 2016/7/30 
				// 在这里加入处理过程记录用户错误密码尝试登录记录
				$login_ip = $_SERVER["REMOTE_ADDR"];
				$GLOBALS['db']->query("insert into ".DB_PREFIX."user_login_log (user_id,create_time,login_ip,password,login_name,state) values(".$user_data['id'].",".time().",'".$login_ip."','','',2)");
				// end
		...
		else
		{
			...
			if($is_use_pass) //未整合，则直接成功
			{
				...
				if ($user_data['sohu_id'] > 0) {
					$GLOBALS['db']->query("update ".DB_PREFIX."user set sohu_id=0 where id='".$user_data['id']."'");
				}
				...
				//记录用户累计密码错误次数
				$GLOBALS['db']->query("update ".DB_PREFIX."user set sohu_id=`sohu_id`+1 where id='".$user_data['id']."'");
				$result['status'] = 0;
				$result['data'] = ACCOUNT_PASSWORD_ERROR;
				$result['times'] = intval($user_data['sohu_id'] + 1);
				return $result;				
			}
		}
	}
	...
	function loginout_user()
	{
		...
		else
		{
			...
			//清除系统session记录
			login_session_state($user_info['id'], es_session::id(), true);
