...
/**
 * 2016-7-13 添加函数,获取有效的下一个月日期 to_next_month
 * 2016-7-13 关闭to_next_month, next_replay_month在多处使用
 * 2016-7-13 对5-31,6-30放款的标的还款计划有影响
 * 2016-7-13 关闭VIP处理,调用函数 syn_user_vip
 * 2016-7-14 新手标还款回款计划完善
 */
...
function to_date($utc_time, $format = 'Y-m-d H:i:s') {
	...
	$timezone = 0;//intval(app_conf('TIME_ZONE'));
	$time = $utc_time + $timezone * 3600; 
	return date ($format, $time );
}

// XHF 2016-7-13
// 关闭时区设置,原有数据的时间均为北京时间
// 当期系统初始使用标准时间
// 便于系统统计
...
/**
 * XHF 新增函数,获取下一个月的时间 2016-7-13
 * 改善系统原有的处理方式
 * @$start_time 开始时间
 * @m 相差月份,默认为1
 * 当日期为29,30,31依据不同的实际月份处理成合理的值
 */
function to_next_month($start_time, $m=1) {
...
function syn_deal_status($id)
{
		...
		if(($deal_info['progress_point'] >=100 || $data['progress_point'] >=100) && floatval($deal_info['load_money']) >=floatval($deal_info['borrow_amount'])){
			...
			elseif(($deal_info['deal_status']==4&&$deal_info['repay_start_time']>0) || ($deal_info['deal_status']==2 && $deal_info['repay_start_time']>0 && $deal_info['repay_start_time'] <= $deals_time)){
				...
				if(!empty($repay_info['all_repay_money'])){
					...
					// XHF 2016-7-13 
                    // 停止中途启用函数 to_next_month
                    // 统一使用系统原有的函数
					$data['next_repay_time'] = next_replay_month($last_repay_time,1,$deal_info['repay_start_time']); 
				}
				...
				//判断是否完成还款
				if($GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."deal_load_repay WHERE has_repay<>1 AND deal_id=$id ") == 0){
					$data['deal_status'] = 5;
				}
				else{
					$data['deal_status'] = 4;
	...
	//发送流标通知
	if(($deal_info['deal_status'] ==3 || (isset($data['deal_status']) && $data['deal_status']==3)) && $deal_info['is_send_bad_msg']==0){
		$data['is_send_bad_msg'] = 1;
		//发邮件和短信
		send_deal_faild_mail_sms($id,$deal_info,$deal_info['user_id']);
		//站内信
	...
	if(app_conf("OPEN_AUTOBID") == 1 && $deal_info['ips_bill_no'] ==""){
		if($deal_info['is_new'] = 0 && ($deal_info['deal_status'] == 1 || $data['deal_status']==1) && $deal_info['remain_time'] >0 && $deals_time - $deal_info['start_time'] >=1800 && $deal_info['progress_point'] <=95 && $deal_info['repay_time_type'] == 1){
...
function sys_user_status($user_id,$is_cache = false,$make_cache=false){
	...
	if($data==false){
		...
		//XHF 2016/8/2 加载用户待收
		$load_wait = $GLOBALS['db']->getRow("SELECT sum(self_money) as total_self_money,sum(repay_money + manage_money + manage_interest_money) as total_repay_money,sum(interest_money) as total_interest_money FROM ".DB_PREFIX."deal_load_repay where ((user_id = ".$user_id." and t_user_id = 0) or t_user_id = ".$user_id.") and has_repay = 0  ");
		//待回收本金
		$data['load_wait_self_money'] = $load_wait['total_self_money'];
		//待回收本息
		$data['load_wait_repay_money'] = $load_wait['total_repay_money'];
		//待回收利息
		$data['load_wait_interest_money'] = $load_wait['total_interest_money'];
...
function send_deal_faild_mail_sms($deal_id,$deal_info=false,$user_id){
	...
	if(app_conf('SMS_ON')==1){
		...
		//尊敬的用户{$notice.user_name},遗憾的通知您,您于{$notice.deal_publish_time}发布的借款"{$notice.deal_name}"流标.
	...
	if($load_user_list){
		...
		//尊敬的用户{$notice.user_name},遗憾的通知您,您于{$notice.deal_publish_time}发布的借款"{$notice.deal_name}"流标,解冻投标资金{$notice.money}元.
...
function send_deal_success_mail_sms($deal_id,$deal_info=false){
	...
	if(app_conf('SMS_ON')==1){
		...
		//尊敬的用户${sh_notice.user_name}，您的借款“${sh_notice.deal_name}” 已满标！
	...
	if($load_user_list){
		...
		foreach($load_user_list as $k=>$v){
			$user_info = get_user_info("*","id = ".$v['user_id']);
			$load_msg_conf = get_user_msg_conf($v['user_id']);
			//XHF 2016/8/11 加入用户邮件判断
			if($load_msg_conf['mail_bidsuccess'] == 1 && app_conf('MAIL_ON')==1 && $user_info['email']){
				$tmpl_content = $load_mail_tmpl['content'];
				$notice['user_name'] = $v['user_name'];
				$notice['deal_name'] = $deal_info['name'];
				$notice['deal_url'] = SITE_DOMAIN.$deal_info['url'];
...
function send_repay_reback_sms_mail($deal_info,$loan_user_info,$unext_loan,$user_load_data,$all_shouyi_money,$all_impose_money){
	if(app_conf("SMS_ON")==1&&app_conf('SMS_REPAY_TOUSER_ON')==1 && $loan_user_info['mobile']!=""){
		//尊敬的用户{$notice.user_name},您所投的标"{$notice.deal_name}"回款{$notice.repay_money}元,感谢您的关注和支持.
		...
		$notice['deal_name'] = $deal_info['sub_name'] != ""? $deal_info['sub_name'] : $deal_info['name'];
...
function send_payment_sms($notice_id)
{
	if(app_conf("SMS_ON")==1&&app_conf("SMS_SEND_PAYMENT")==1)
	{
		...
		if($notice_data)
		{
			...
			{
				//尊敬的用户{$payment_notice.user_name},您所下订单{$payment_notice.notice_sn}金额{$payment_notice.money_format}于{$payment_notice.pay_time_format}支付成功.
				...
				//json数据
				$msg_data['datas'] = serialize(array(
					'user_name' => $notice_data['user_name'],
					'notice_sn' => $notice_data['notice_sn'],
					'money_format' => $notice_data['money_format'],
					'pay_time_format' => $notice_data['pay_time_format']
				));
				//end
...
/**
 * XHF 2016/8/6 修改短信发送
 * $msg_item['datas'] 模板参数集合,json格式
 * $msg_item['template_id'] 模板ID
 * $msg_item['content'] 兼容直接发送内容
 */
function send_sms_email($msg_item){
	...
	if($msg_item['send_type']==0)
	{
		...
		*/
		$result = $sms->sendTemplateSMS($msg_item['dest'],$msg_item['datas'],$msg_item['template_id'],$msg_item['content']);
		//end
		
		//发送结束，更新当前消息状态
		$GLOBALS['db']->query("update ".DB_PREFIX."deal_msg_list set is_success = ".intval($result['status']).",result='".$result['msg']."',send_time='".TIME_UTC."' where id =".intval($msg_item['msg_id']));
	...
	if($msg_item['send_type']==1)
	{
		...
		//发送结束，更新当前消息状态
		$GLOBALS['db']->query("update ".DB_PREFIX."deal_msg_list set is_success = ".intval($is_success).",result='".$result."',send_time='".TIME_UTC."' where id =".intval($msg_item['msg_id']));
 ...
 * bday 出生日期
 */
function idCardSubInfo($idno) {
	$data = array();
	if (!getIDCardInfo($idno)) {
		return null;
	}

	$len = strlen($idno);
	$data['sex'] = intval(($len == 18)? substr($idno,16,1) : substr($idno,14,1)) % 2;
	$data['byear'] = ($len == 18)? substr($idno,6,4) : ('19' . substr($idno,6,2));
	$data['bmonth'] = ($len == 18)? substr($idno,10,2) : ('19' . substr($idno,8,2));
	$data['bday'] = ($len == 18)? substr($idno,12,2) : ('19' . substr($idno,10,2));
	return $data;
}
 ...
 * @param 身份证 $idcard
 */
function idCardVerify($realname, $idno) {
	$db_modules = $GLOBALS['db']->getAll("select class_name, is_effect from ".DB_PREFIX."idcard where is_effect=1");
	$result = array('status'=>0, 'isok'=>false, 'info'=>'系统暂停在线实名认证功能');

	if ($db_modules && is_array($db_modules) && count($db_modules)) {
		$class_name = $db_modules[0]['class_name']."_idcard";
		require_once (APP_ROOT_PATH."system/idcard/".$class_name.".php");

		$obj = new $class_name();
		$result = $obj->getinfo(array('card'=>$idno, 'realname'=>$realname));
	}
	return $result;
}
...
function pay_manage_referrals($deal,$loan){
	...
	//部门成员
	$mymanager = $GLOBALS['db']->getRow("SELECT referrals_rate,pid FROM ".DB_PREFIX."admin WHERE id=".$user_info['admin_id']);
 ...
 */
 function do_loans($id,$repay_start_time,$type = 0){
	...
	if($loan_data['repay_start_time'] > 0 && $loan_data['repay_time_type'] == 0){
		$deal_info['next_repay_time'] = $loan_data['next_repay_time'] = $loan_data['repay_start_time'] + $loan_data['repay_time']*24*3600;
	} else {
        // XHF 2016-7-13
        // 停止中途启用函数 to_next_month
        // 统一使用系统原有的函数
	...
	if($GLOBALS['db']->affected_rows() > 0){
		...
		$load_list = $GLOBALS['db']->getAll("SELECT id,user_id,`money`,`is_old_loan`,`rebate_money`,`bid_score`,`is_winning`,`income_type`,`income_value`,`create_time` FROM ".DB_PREFIX."deal_load where deal_id=".$id." and is_rebate = 0 ");
		...
		foreach($load_list as $lk=>$lv){
			...
			if(floatval($lv["rebate_money"])!=0 || intval($lv["bid_score"])!=0){
				...
				if($GLOBALS['db']->affected_rows()){
					...
					if(((int)$lv['income_type']==1 || (int)$lv['income_type']==2 || (int)$lv['income_type']==3 || (int)$lv['income_type']==4) && $lv['is_winning']==1){
						...
						if($is_send_mail == 1 && app_conf("MAIL_ON")==1 && $user_info['email'] != "")
						{
						...
						{
							//礼品奖励,积分奖励或非现金奖励通知模板
							//尊敬的用户{$notice.user_name},您的投标"{$notice.deal_name}"获得礼品奖励,礼品{$notice.gift_value},奖励已于{$notice.release_date}发送.
            ...
            // 申购期利息自动计算转至客户帐户 XHF 2016-6-30
            // 原始方法人工处理
            // 按约定收益率及间隔天数计算收益
            // 满标放款前收益
            // XHF 2016-7-14,申购期利息关联借款标的便于统计,
            // 添加借款标的对应id,对应user_money_log.deal_id
            $daytimediff = strtotime(date('Y-m-d',time())) - strtotime(date('Y-m-d',$lv['create_time']));
            $days = $daytimediff / 86400;
            if ($days > 0) {
                $pre_load_money = round($lv['money'] * $days * ($deal_info['rate'] / 100 / 12 / 30),2);
                modify_account(array("money"=>$pre_load_money,"deal_id"=>$deal_info['id']),$lv['user_id'],"[<a href='".$deal_info['url']."' target='_blank'>".$deal_info['name']."</a>],申购期收益",54);
            }
            // end 2016-6-30
		}
		
		//VIP升级 -借款
        //关闭VIP升级-借款 XHF 2016-7-13
        /*
		$type = 1;
		$type_info = 1;
...
function do_received($id,$type = 0,$bad_msg = ""){
	...
	if($type == 0){
		...
		foreach($r_load_list as $k=>$v){
			...
			if($v['ecv_id'] > 0)
				$GLOBALS['db']->query("UPDATE ".DB_PREFIX."ecv SET use_count=use_count - 1 WHERE ecv_id=".$v['ecv_id']);
		}
	}
	else{
		$r_load_list = $GLOBALS['db']->getAll("SELECT id,user_id,money,is_old_loan,ecv_id FROM ".DB_PREFIX."deal_load WHERE is_repay=0 AND deal_id=$id");
		foreach($r_load_list as $k=>$v){
			$GLOBALS['db']->query("UPDATE ".DB_PREFIX."deal_load SET is_repay=1 WHERE id=$id");
			if($v['ecv_id'] > 0)
				$GLOBALS['db']->query("UPDATE ".DB_PREFIX."ecv SET use_count=use_count - 1 WHERE ecv_id=".$v['ecv_id']);
		}
	}
...
function isMobile() {
	...
	// XHF 2016-07-16
	// add agent: android,linux,iphone ...  
	$mobile_agents = array(   
	    ...
	    'wapr','webc','winw','winw','xda','xda-','andr','linu','ipho'  
	    );   
...
function hideUserName($user_name){
	 if ($user_name == "") { return ""; }

	 if (preg_match("/^1[34578][0-9]{9}$/", $user_name))
		return preg_replace('#(\d{3})\d{5}(\d{3})#', '${1}*****${2}',$user_name);
	 else 
		return substr($user_name,0,2).'*****'.substr($user_name,-2);
}
...
/**
 * VIP 升降级
 * XHF 2016/8/22 原始vip升级定义
 */
function syn_user_vip_old($user_id,$type,$type_info,$vip_id){
    // 关闭VIP等级处理 XHF 2016-7-13
    // vip_id=2的用户 2016-7-13 投标计算出的用户收益与正常不符
    // 关闭vip更新功能
    return array();
    // end

 	$vipdata = array();
 	//$type_info 1借款升级、2投标升级、3正常还款升级、4提前还款升级、5.逾期还款降级、6.代还款降级、7.坏账降级、8.VIP购买升级、9.VIP购买到期降级
	switch($type_info){
		case "1":
			$vipdata['causes']="借款升级";
			break;
 ...
 * $type_info 1借款升级、2投标升级、3正常还款升级、4提前还款升级、5.逾期还款降级、6.代还款降级、7.坏账降级、8.VIP购买升级、9.VIP购买到期降级
 */
function syn_user_vip($user_id,$type,$type_info,$vip_id){
	//暂不支持VIP降级
	if ($type == 2) { return array(); }
	//暂不支持除投标升级的其他操作
	if ($type_info != 2) { return array(); }
	...
	switch($type_info){
 	...
 	$vipdata['user_id'] = $user_id;
	$user_info = get_user_info("*","id='$user_id' ");
	$load_total = $GLOBALS['db']->getOne("select sum(dl.money) FROM ".DB_PREFIX."deal_load dl LEFT JOIN ".DB_PREFIX."deal d ON dl.deal_id=d.id WHERE dl.user_id='$user_id' and d.deal_status > 3 ");

	$gradeinfo = $GLOBALS['db']->getRow("select * FROM ".DB_PREFIX."vip_type WHERE lower_limit<='$load_total' and upper_limit>'$load_total' and is_effect='1' and is_delete='0' ");
 	
	//未找到对应的VIP类别或用户的VIP等级未改变
	if (!$gradeinfo || $gradeinfo['id'] == $user_info['vip_id']) {
		return array();
	}
 ...
 * @param  boolean $add 是否追加模式
 * @return 无返回值
 */
function save_log_ext($file_name,$str,$add=true) {
    if (file_exists($file_name) && $add == true) {
        $str = file_get_contents($file_name).$str;
    }
