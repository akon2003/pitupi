	...
	function make_repay_plan($deal){
		...
		for($i=0;$i<$true_repay_time;$i++){
			...
			if($deal['repay_time_type']==0)
				...
				$repay_day = $load_repay['repay_time'] = to_next_month($repay_day,$all_repay_time);
		...
		foreach($load_users as $k=>$v){
			...
			if($idx+1 == $true_repay_time){
                ...
                if($k+1 == count($load_users)){
                    ...
                    $item['mortgage_fee']= $deal['all_manage_money'] / $deal['mortgage_fee'];
                }
			}
	...
	function inrepay_repay($loaninfo,$k,$time_utc=0){
		...
		if($loaninfo['deal']['repay_time_type'] == 0){
			...
			for($i=0;$i<$all_repay_time;$i++){
				if($time_utc >= to_next_month($loaninfo['deal']['repay_start_time'],$i)+24*3600-1 ){
					$true_k +=1;
				}
			}
			$true_m = $true_k;
