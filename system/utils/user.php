	...
	{		
		...
		if($GLOBALS['db']->autoExecute(DB_PREFIX."user",$user,$mode,$where))
		{
			if($mode == 'INSERT')
			{
				...
				{
					if ($register_score > 0)    { $user_get['score'] = $register_score; }
					//if ($register_money > 0)    { $user_get['money'] = $register_money; }
					if ($register_point > 0)    { $user_get['point'] = $register_point; }
					//if ($register_lock_money > 0) { $user_get['reg_lock_money'] = $register_lock_money; }
